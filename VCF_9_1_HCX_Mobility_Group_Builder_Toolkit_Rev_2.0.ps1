<#
.SYNOPSIS
  VCF 91 HCX MobilityGroup Builder Toolkit Rev 3.1b-Network-Merge-Runtime-Fix.ps1
.DESCRIPTION
...
function Convert-ToNicMappings($VmRaw) {
    ...
    foreach($nic in @(Get-NestedNics $VmRaw)){
        ...
        $resolution=Resolve-HcxDestinationNetworkMatch -SourceName $srcName -SourceId $srcId
        ...
        $result.Add([pscustomobject]@{
            ...
            DestinationNetworkType=if($dst){[string]$dst.EntityType}else{''};MatchStatus=[string]$resolution.Status;MatchDetail=[string]$resolution.Detail
        })
    }
    return $result
}
...
function Apply-GlobalSelection {
    ...
    foreach($row in @($script:Rows)){
        ...
        if($sp){$row.StoragePolicy=[string]$sp.Name}
...
function P2-Inventory{
 ...
 if($script:cmbP2Storage.Items.Count){$script:cmbP2Storage.SelectedIndex=0};if($script:cmbP2Compute.Items.Count){$script:cmbP2Compute.SelectedIndex=0};if($script:cmbP2Policy.Items.Count){$script:cmbP2Policy.SelectedIndex=0};Log "Phase II discovery loaded storage=$($(Get-SafeCount $storage)), compute=$($(Get-SafeCount $compute))." PASS
}
...
function P2-Create{
 if(-not$script:P2Valid){throw 'Run validation first.'};$null=Assert-HcxAutomaticDiscoveryCurrent;if(-not$script:P2Payload){$script:P2Payload=@(P2-BuildAll)};$summary=@($script:P2Payload|ForEach-Object{"$($_.Name): $($_.VMCount) VM(s)"})-join[Environment]::NewLine;if([Windows.MessageBox]::Show("Create these HCX drafts one at a time?`n`n$summary",'Create Mobility Groups','YesNo')-ne'Yes'){return};$folder=script:Get-P2ArtifactFolder;$ok=0
...
Log 'Rev 3.1b runtime-safe deterministic network auto-match, row dropdown binding, validation state, case-sensitive HCX JSON, resilient HTTP transport, and default verbose diagnostics active: current-session topology, direction, site-pair, Service Mesh, vCenter and NSX network discovery, scalar Count normalization, Phase I/II diagnostics, and placement normalization.' PASS
