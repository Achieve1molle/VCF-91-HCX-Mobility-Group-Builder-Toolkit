<#
.SYNOPSIS
  VCF 9.1 HCX Mobility Group Builder Toolkit Rev 3.5.1-Override-Control-Fix.ps1
.DESCRIPTION
...
# Self-signing disabled for this development revision. Sign manually after validation.
...
$script:LastCsv = $null
$script:LastVmImportReport = $null
$script:SchemaVersion = '2.0'
$script:MobilityGroupMaximum = 50
$script:MasterIncludeSync = $false
$script:P2Schedules = [Collections.ObjectModel.ObservableCollection[object]]::new()
$script:CurrentBuildingGroupNumber = 0
$script:SuppressScheduleRefresh = $false
$script:PolicySelectionsInitialized = $false
$script:ApplyingStorageDefaults = $false
$script:StorageDefaultsApplied = $false
...
function Update-Prerequisites {
    ...
    $srcConnected=[bool]$script:SourceVIServer;$dstConnected=[bool]$script:DestinationVIServer
    $script:lblSourceVCStatus.Text=if($srcConnected){"Connected - $($script:SourceVIServer.Name)"}else{'Not connected'};$script:lblSourceVCStatus.Foreground=if($srcConnected){'LightGreen'}else{'#76C7D8'}
    $script:lblDestinationVCStatus.Text=if($dstConnected){"Connected - $($script:DestinationVIServer.Name)"}else{'Not connected'};$script:lblDestinationVCStatus.Foreground=if($dstConnected){'LightGreen'}else{'#76C7D8'}
}
function Show-HcxThemedNotice {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$Title,
        [Parameter(Mandatory)][string]$Heading,
        [Parameter(Mandatory)][string]$Message,
        [ValidateSet('Success','Information','Warning','Error')][string]$Kind='Information',
        [string]$Details=''
    )
    $accent=switch($Kind){'Success'{'#72D572'};'Warning'{'#F2C94C'};'Error'{'#FF6B6B'};default{'#76C7D8'}}
    $x=@'
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" Title="HCX 9.1 Mobility Group Builder" Height="330" Width="680" MinHeight="270" MinWidth="560" WindowStartupLocation="CenterOwner" ResizeMode="CanResizeWithGrip" Background="#071015" Foreground="#E6E6E6" FontFamily="Segoe UI" ShowInTaskbar="False">
<Grid Margin="16"><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="*"/><RowDefinition Height="Auto"/></Grid.RowDefinitions>
<Border x:Name="banner" Background="#10242D" BorderThickness="1" CornerRadius="4" Padding="13"><StackPanel><TextBlock x:Name="heading" FontSize="18" FontWeight="SemiBold"/><TextBlock x:Name="subtitle" Margin="0,5,0,0" TextWrapping="Wrap" Foreground="#B8CBD3"/></StackPanel></Border>
<Border Grid.Row="1" Background="#0D1B22" BorderBrush="#314A57" BorderThickness="1" CornerRadius="3" Margin="0,12,0,0" Padding="12"><TextBlock x:Name="message" FontSize="14" TextWrapping="Wrap"/></Border>
<TextBox x:Name="details" Grid.Row="2" Margin="0,10,0,0" IsReadOnly="True" TextWrapping="Wrap" VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Disabled" Background="#071015" Foreground="#76C7D8" BorderBrush="#314A57" Padding="10"/>
<Button x:Name="ok" Grid.Row="3" Content="OK" Width="110" Height="32" HorizontalAlignment="Right" Margin="0,12,0,0" Background="#2B3740" Foreground="#F1F4F6" BorderBrush="#76C7D8" FontWeight="SemiBold" IsDefault="True" IsCancel="True"/>
</Grid></Window>
'@
    $reader=[Xml.XmlNodeReader]::new([xml]$x);$dialog=[Windows.Markup.XamlReader]::Load($reader)
    if($script:Window){$dialog.Owner=$script:Window}
    $subtitleText = switch ($Kind) {
        'Success'     { 'Operation completed successfully' }
        'Warning'     { 'Review recommended' }
        'Error'       { 'Operation could not be completed' }
        default       { 'Inventory and connection status' }
    }
    $dialog.Title=$Title
    $dialog.FindName('heading').Text=$Heading
    $dialog.FindName('subtitle').Text=$subtitleText
    $dialog.FindName('message').Text=$Message
    $dialog.FindName('details').Text=$Details
    $detailsVisibility = if ([string]::IsNullOrWhiteSpace($Details)) { 'Collapsed' } else { 'Visible' }
    $dialog.FindName('details').Visibility=$detailsVisibility
    $dialog.FindName('banner').BorderBrush=$accent;$dialog.FindName('heading').Foreground=$accent;$dialog.FindName('ok').BorderBrush=$accent
    $dialog.FindName('ok').Add_Click({$dialog.DialogResult=$true;$dialog.Close()})
...
function Disconnect-HcxRest {
    ...
    if($script:SourceVIServer){Disconnect-VIServer -Server $script:SourceVIServer -Force -Confirm:$false -ErrorAction SilentlyContinue|Out-Null};if($script:DestinationVIServer -and $script:DestinationVIServer -ne $script:SourceVIServer){Disconnect-VIServer -Server $script:DestinationVIServer -Force -Confirm:$false -ErrorAction SilentlyContinue|Out-Null};$script:SourceVIServer=$null;$script:DestinationVIServer=$null;if($script:lblSourceVCStatus){Update-Prerequisites}
...
function Bind-DestinationControls {
    try{
        $previousGlobalPolicy=if($script:PolicySelectionsInitialized){[string]$script:cmbGlobalStoragePolicy.Text}else{''}
        $previousVtpmPolicy=if($script:PolicySelectionsInitialized){[string]$script:cmbVtpmStoragePolicy.Text}else{''}
        ...
        $globalPolicy=@($script:Inventory.Policies|Where-Object Name -ieq $previousGlobalPolicy|Select-Object -First 1)
        if(-not$globalPolicy.Count){$globalPolicy=@($script:Inventory.Policies|Where-Object Name -ieq 'Datastore Default Storage Policy'|Select-Object -First 1)}
        if($globalPolicy.Count){$script:cmbGlobalStoragePolicy.SelectedItem=$globalPolicy[0]}
        $vtpmPolicy=@($script:Inventory.Policies|Where-Object Name -ieq $previousVtpmPolicy|Select-Object -First 1)
        if($vtpmPolicy.Count){$script:cmbVtpmStoragePolicy.SelectedItem=$vtpmPolicy[0]}elseif(-not$script:PolicySelectionsInitialized){$script:cmbVtpmStoragePolicy.SelectedIndex=-1}
        $script:PolicySelectionsInitialized=$true
        Log "Destination policy selection restored: Standard='$($script:cmbGlobalStoragePolicy.Text)'; vTPM='$($script:cmbVtpmStoragePolicy.Text)'." INFO
...
$script:Inventory.Policies=@(Get-SpbmStoragePolicy -Server $script:DestinationVIServer -ErrorAction SilentlyContinue | Sort-Object Name | ForEach-Object{[pscustomobject]@{Name=[string]$_.Name;Id=[string]$_.Id;PolicyValue=[string]$_.Id;PolicyType='defined';ProfileType='SPBM';Source='vCenter SPBM';Raw=$_}})
...
function ConvertTo-HcxStoragePolicyRecord {
    param($Item)
    if($null-eq$Item){return $null}
    $name=[string](Get-HcxFirstValue $Item @('name','displayName'))
    $entityId=[string](Get-HcxFirstValue $Item @('entityId','id','uuid'))
    $object=Get-HcxPropertyValue $Item 'object'
    $type=[string](Get-HcxFirstValue $object @('type'))
    $profileType=[string](Get-HcxFirstValue $object @('profileType'))
    if([string]::IsNullOrWhiteSpace($name)){return $null}
    if($name -ieq 'Datastore Default Storage Policy' -or $type -ieq 'default'){
        $entityId='default';$type='default'
    }elseif([string]::IsNullOrWhiteSpace($type)){$type='defined'}
    [pscustomobject]@{Name=$name;Id=$entityId;PolicyValue=$entityId;PolicyType=$type;ProfileType=$profileType;Source='HCX';Raw=$Item}
}
function Get-HcxStoragePolicyInventory {
    $null=Assert-HcxAutomaticDiscoveryCurrent
    $destination=$script:HcxDiscovery.Destination
    $path='/hybridity/api/v2/inventory/storageprofiles?hcspUUID='+[uri]::EscapeDataString([string]$script:HcxDiscovery.HcspUUID)
    $body=[ordered]@{infraManagerId=[string]$destination.ResourceId;endpointId=[string]$destination.EndpointId}
    $response=Invoke-HcxRest -Method POST -Path $path -Body $body -AllowFailure
    $records=@()
    if($response){$records=@(Get-HcxResponseItems $response|ForEach-Object{ConvertTo-HcxStoragePolicyRecord $_}|Where-Object{$_})}
    if(-not@($records|Where-Object Name -ieq 'Datastore Default Storage Policy').Count){
        $records+= [pscustomobject]@{Name='Datastore Default Storage Policy';Id='default';PolicyValue='default';PolicyType='default';ProfileType='DEFAULT';Source='HCX Default';Raw=$null}
        Log 'HCX storage-profile inventory did not return Datastore Default Storage Policy; the HCX default profile descriptor was added with value=default and type=default.' WARN
    }
    return @($records|Sort-Object Name -Unique)
}
function Merge-HcxStoragePolicyInventory {
    $hcxPolicies=@(Get-HcxStoragePolicyInventory)
    $spbm=@($script:Inventory.Policies|Where-Object{[string]$_.Source -ne 'HCX' -and [string]$_.Source -ne 'HCX Default'}|ForEach-Object{
        $id='';foreach($pn in 'Id','Uid','UniqueId'){$prop=$_.PSObject.Properties[$pn];if($prop-and$prop.Value){$id=[string]$prop.Value;break}}
        [pscustomobject]@{Name=[string]$_.Name;Id=$id;PolicyValue=$id;PolicyType='defined';ProfileType='SPBM';Source='vCenter SPBM';Raw=$_}
    })
    $byName=@{}
    foreach($policy in @($spbm)+@($hcxPolicies)){
        $key=$policy.Name.ToLowerInvariant()
        if(-not$byName.ContainsKey($key)-or$policy.Source -like 'HCX*'){$byName[$key]=$policy}
    }
    $script:Inventory.Policies=@($byName.Values|Sort-Object Name)
    Bind-DestinationControls
    Log "Merged HCX storage-profile inventory: HCX=$(@($hcxPolicies).Count); Combined=$(@($script:Inventory.Policies).Count); DatastoreDefault=$([bool]@($script:Inventory.Policies|Where-Object Name -ieq 'Datastore Default Storage Policy').Count)." PASS
}
function Get-P2StoragePolicyDescriptor([string]$Name){
    $policy=@($script:Inventory.Policies|Where-Object Name -ieq $Name|Select-Object -First 1)
    if($policy.Count){
        $value=[string]$policy[0].PolicyValue;if([string]::IsNullOrWhiteSpace($value)){$value=[string]$policy[0].Id}
        $type=[string]$policy[0].PolicyType;if([string]::IsNullOrWhiteSpace($type)){$type='defined'}
        if($Name -ieq 'Datastore Default Storage Policy'){$value='default';$type='default'}
        return [pscustomobject]@{Name=[string]$policy[0].Name;Value=$value;Type=$type;Source=[string]$policy[0].Source}
    }
    if($Name -ieq 'Datastore Default Storage Policy'){return [pscustomobject]@{Name=$Name;Value='default';Type='default';Source='HCX Default'}}
    $legacy=Get-P2StoragePolicyId $Name
    if($legacy){return [pscustomobject]@{Name=$Name;Value=$legacy;Type='defined';Source='vCenter SPBM'}}
    return $null
}
...
function Set-StorageDefaultsStatus([string]$Text,[string]$Color='#76C7D8') {
    if($script:lblStorageDefaultsStatus){$script:lblStorageDefaultsStatus.Text=$Text;$script:lblStorageDefaultsStatus.Foreground=$Color}
}
function Set-StorageDefaultsPending([string]$Reason='Global storage selection changed') {
    if($script:ApplyingStorageDefaults){return}
    $script:StorageDefaultsApplied=$false
    Set-StorageDefaultsStatus 'Pending - select Apply Storage Defaults' 'Gold'
    Invalidate-Validation $Reason
    Invalidate-P2Validation $Reason
}
function Apply-StorageDefaultsToIncludedVMs {
    Commit-Grid
    $storage=$script:cmbGlobalDatastore.SelectedItem
    $standard=$script:cmbGlobalStoragePolicy.SelectedItem
    $vtpm=$script:cmbVtpmStoragePolicy.SelectedItem
    $disk=if($script:cmbGlobalDiskFormat.SelectedItem -is [Windows.Controls.ComboBoxItem]){[string]$script:cmbGlobalDiskFormat.SelectedItem.Content}else{[string]$script:cmbGlobalDiskFormat.Text}
    if(-not$storage){throw 'Select a Storage Target before applying storage defaults.'}
    if(-not$standard){throw 'Select a Standard Policy before applying storage defaults.'}
    $included=@($script:Rows|Where-Object Include)
    if(-not$included.Count){throw 'No included VM rows are available for storage-default application.'}
    $preserveOverrides=[bool]$script:chkPreserveStorageOverrides.IsChecked
    $existingOverrides=@($included|Where-Object{[string]$_.StoragePolicyAssignment -eq 'Per-VM Override'})
    if(-not$preserveOverrides -and $existingOverrides.Count){
        $answer=[Windows.MessageBox]::Show("Replace $($existingOverrides.Count) per-VM policy override(s) with the selected Standard or vTPM global policy?`n`nThis changes intentional VM-specific policy exceptions.",'Replace Per-VM Policy Overrides','YesNo','Warning')
        if($answer -ne 'Yes'){Log 'Storage-default application canceled before replacing per-VM policy overrides.' WARN;return}
    }
    $vtpmRows=@($included|Where-Object{[int]$_.Vtpm -eq 1 -and (-not$preserveOverrides -or [string]$_.StoragePolicyAssignment -ne 'Per-VM Override')})
    if($vtpmRows.Count -and -not$vtpm){throw "$($vtpmRows.Count) included vTPM VM(s) require a selected vTPM Policy."}
    $standardUpdated=0;$vtpmUpdated=0;$overridesPreserved=0;$overridesReplaced=0
    $script:ApplyingStorageDefaults=$true
    try{
        foreach($row in $included){
            $row.DestinationDatastore=[string]$storage.Name
            $row.DestinationDatastoreId=[string]$storage.Id
            $row.DestinationStorageType=[string]$storage.Type
            $row.DatastoreSelectionSource='Global'
            $wasOverride=([string]$row.StoragePolicyAssignment -eq 'Per-VM Override')
            if($wasOverride -and $preserveOverrides){$overridesPreserved++}
            elseif([int]$row.Vtpm -eq 1){$row.StoragePolicy=[string]$vtpm.Name;$row.StoragePolicyAssignment='vTPM Global';$vtpmUpdated++;if($wasOverride){$overridesReplaced++}}
            else{$row.StoragePolicy=[string]$standard.Name;$row.StoragePolicyAssignment='Standard Global';$standardUpdated++;if($wasOverride){$overridesReplaced++}}
            if($disk){$row.DiskFormat=$disk;$row.DiskFormatSelectionSource='Phase I Global'}
            $row.ValidationMessage='Not validated - storage defaults changed'
        }
        $script:gridStorageRows.Items.Refresh();$script:gridVMs.Items.Refresh()
    }finally{$script:ApplyingStorageDefaults=$false}
    $script:StorageDefaultsApplied=$true
    $script:P2Payload=$null
    Invalidate-Validation 'Storage defaults applied to included VM rows'
    Invalidate-P2Validation 'Storage defaults applied to included VM rows'
    Set-StorageDefaultsStatus "Applied - Standard=$standardUpdated; vTPM=$vtpmUpdated; Overrides preserved=$overridesPreserved; Overrides replaced=$overridesReplaced" 'LightGreen'
    Log "STORAGE DEFAULTS APPLIED: Included='$($included.Count)'; StandardUpdated='$standardUpdated'; vTPMUpdated='$vtpmUpdated'; OverridesPreserved='$overridesPreserved'; OverridesReplaced='$overridesReplaced'; PreserveOverrides='$preserveOverrides'; Storage='$($storage.Name)'; StandardPolicy='$($standard.Name)'; vTPMPolicy='$([string]$vtpm.Name)'; DiskFormat='$disk'." PASS
    Show-HcxThemedNotice -Title 'HCX Storage Defaults' -Heading 'Storage defaults applied' -Kind Success -Message "Updated $($included.Count-$overridesPreserved) included VM policy assignment(s) and $($included.Count) storage target(s)." -Details ("Standard Policy: $($standard.Name)`nStandard VMs updated: $standardUpdated`nvTPM Policy: $([string]$vtpm.Name)`nvTPM VMs updated: $vtpmUpdated`nPer-VM policy overrides preserved: $overridesPreserved`nPer-VM policy overrides replaced: $overridesReplaced`nStorage Target: $($storage.Name)`nDisk Format: $disk")
}
function Test-Phase1StoragePolicyConsistency($Rows,[switch]$ThrowOnError) {
    $issues=[Collections.Generic.List[string]]::new()
    $standard=[string]$script:cmbGlobalStoragePolicy.Text
    $vtpm=[string]$script:cmbVtpmStoragePolicy.Text
    foreach($row in @($Rows|Where-Object Include)){
        if([string]$row.StoragePolicyAssignment -eq 'Per-VM Override'){continue}
        if([int]$row.Vtpm -eq 1){
            if([string]::IsNullOrWhiteSpace($vtpm)){$issues.Add("$($row.VMName): vTPM is detected but no global vTPM Policy is selected.")}
            elseif([string]$row.StoragePolicy -ine $vtpm){$issues.Add("$($row.VMName): vTPM Global assignment uses '$($row.StoragePolicy)' but selected vTPM Policy is '$vtpm'. Select Apply Storage Defaults.")}
            elseif([string]$row.StoragePolicyAssignment -ne 'vTPM Global'){$issues.Add("$($row.VMName): vTPM VM has unexpected policy assignment '$($row.StoragePolicyAssignment)'.")}
        }else{
            if([string]::IsNullOrWhiteSpace($standard)){$issues.Add("$($row.VMName): no global Standard Policy is selected.")}
            elseif([string]$row.StoragePolicy -ine $standard){$issues.Add("$($row.VMName): Standard Global assignment uses '$($row.StoragePolicy)' but selected Standard Policy is '$standard'. Select Apply Storage Defaults.")}
            elseif([string]$row.StoragePolicyAssignment -ne 'Standard Global'){$issues.Add("$($row.VMName): standard VM has unexpected policy assignment '$($row.StoragePolicyAssignment)'.")}
        }
    }
    if($issues.Count){
        Set-StorageDefaultsStatus "Mismatch - $($issues.Count) VM(s)" 'Tomato'
        foreach($issue in $issues){Log "STORAGE POLICY CONTRACT: $issue" ERROR}
        if($ThrowOnError){throw "Storage policy reconciliation failed for $($issues.Count) VM(s).`n`n$($issues -join [Environment]::NewLine)"}
        return @($issues)
    }
    Set-StorageDefaultsStatus 'Applied and reconciled' 'LightGreen'
    Log "STORAGE POLICY CONTRACT PASSED: Included='$(@($Rows|Where-Object Include).Count)'; StandardPolicy='$standard'; vTPMPolicy='$vtpm'." PASS
    return @()
}
...
function Validate-Phase1 {
    ...
    foreach($r in @($script:Rows)){
        ...
        if([string]$r.StoragePolicyAssignment -ne 'Per-VM Override'){
            if([int]$r.Vtpm -eq 1){$expected=[string]$script:cmbVtpmStoragePolicy.Text;if([string]::IsNullOrWhiteSpace($expected)){$rowErrors.Add('Detected vTPM requires a selected global vTPM Policy.')}elseif([string]$r.StoragePolicy -ine $expected){$rowErrors.Add("vTPM Global policy '$($r.StoragePolicy)' does not match selected vTPM Policy '$expected'. Select Apply Storage Defaults.")}}
            else{$expected=[string]$script:cmbGlobalStoragePolicy.Text;if([string]::IsNullOrWhiteSpace($expected)){$rowErrors.Add('A global Standard Policy is required.')}elseif([string]$r.StoragePolicy -ine $expected){$rowErrors.Add("Standard Global policy '$($r.StoragePolicy)' does not match selected Standard Policy '$expected'. Select Apply Storage Defaults.")}}
        }
...
function Export-Phase1Csv {
    ...
    $null=Test-Phase1StoragePolicyConsistency -Rows $script:Rows -ThrowOnError
...
function Use-CurrentVmListForMobilityGroups {
    [CmdletBinding()]
    param([switch]$PreserveValidation)
    Commit-Grid
    $null=Test-Phase1StoragePolicyConsistency -Rows $script:Rows -ThrowOnError
    if(-not@($script:Rows).Count){throw 'Import VMs before selecting Use Current VM List.'}
    $script:P2Rows.Clear()
    foreach($r in @($script:Rows|Where-Object Include)){
        $net=@($r.NicMappings|ForEach-Object{[pscustomobject]@{Adapter=$_.Adapter;SourceNetworkName=$_.SourceNetworkName;SourceNetworkId=$_.SourceNetworkId;DestinationNetworkName=$_.DestinationNetworkName;DestinationNetworkId=$_.DestinationNetworkId;DestinationNetworkType=$_.DestinationNetworkType;MatchStatus=$_.MatchStatus;MatchDetail=$_.MatchDetail;MappingSource=$_.MappingSource}})
        $script:P2Rows.Add([pscustomobject]@{
            Include=$true;VMName=$r.VMName;VMId=$r.VMId;MigrationType=$r.MigrationType;Compute=$r.DestinationCompute;ComputeId=$r.DestinationComputeId;ComputeType=$r.DestinationComputeType
            MobilityGroupNumber=[int]$r.MobilityGroupNumber;Folder=$r.DestinationFolder;FolderId=$r.DestinationFolderId
            StorageType=$r.DestinationStorageType;Storage=$r.DestinationDatastore;StorageId=$r.DestinationDatastoreId;StoragePolicy=$r.StoragePolicy
            Vtpm=if($null-ne$r.Vtpm){[int]$r.Vtpm}else{0};StoragePolicyAssignment=$r.StoragePolicyAssignment;StorageSelectionSource='Current Working Set'
            DiskFormat=$r.DiskFormat;DiskFormatSelectionSource=$r.DiskFormatSelectionSource;Networks=@($net)
            NetworkSummary=(@($net|ForEach-Object{"$($_.SourceNetworkName) -> $($_.DestinationNetworkName)"})-join'; ')
        })
    }
    if(-not$script:P2Rows.Count){throw 'No VMs are currently selected for inclusion.'}
    $script:gridP2.ItemsSource=$script:P2Rows
    $groups=@($script:P2Rows|ForEach-Object{[int]$_.MobilityGroupNumber});$max=if($groups.Count){[int](($groups|Measure-Object -Maximum).Maximum)}else{1}
    $script:cmbP2GroupCount.SelectedItem=$max
    $script:txtP2Csv.Text='Current VM list - validation required'
    $script:txtP2Hcx.Text=[string]$script:Hcx.BaseUri
    $script:txtP2SourceSite.Text=if($script:SourceVIServer){[string]$script:SourceVIServer.Name}else{[string]$script:txtSourceVC.Text}
    $script:txtP2DestSite.Text=if($script:DestinationVIServer){[string]$script:DestinationVIServer.Name}else{[string]$script:txtDestinationVC.Text}
    if(-not$script:txtP2Name.Text){$script:txtP2Name.Text='HCX Mobility Group '+(Get-Date -Format yyyyMMdd-HHmm)}
    P2-Invalidate
    if(-not$PreserveValidation){
        Invalidate-Validation 'Current VM list selected as the Mobility Group data source; explicit validation is required'
        $script:lblValidation.Text='Not validated - select Validate Configuration'
        $script:lblValidation.Foreground='#76C7D8'
    }else{
        $script:txtP2Csv.Text='Current validated VM list'
    }
    Update-MasterIncludeState P2;Refresh-P2ScheduleRows
    $validationState=if($PreserveValidation){'complete'}else{'required'}
    Log "Loaded $($script:P2Rows.Count) VM(s) from the current working set. Explicit configuration validation is $validationState." INFO
}
function Validate-SelectedMobilityGroupData {
    $source=[string]$script:txtP2Csv.Text
    if($source -like 'Current VM list*' -or $source -eq 'Current validated VM list'){
        if(Validate-Phase1){
            Use-CurrentVmListForMobilityGroups -PreserveValidation
            $script:lblValidation.Text='Pass - current VM configuration'
            $script:lblValidation.Foreground='LightGreen'
            Log 'Explicit configuration validation passed and the current Mobility Group working set was refreshed.' PASS
            return $true
        }
        return $false
    }
    if((Test-Path -LiteralPath $source -PathType Leaf) -and $script:P2Rows.Count -gt 0){
        $script:ValidationCurrent=$true
        $script:btnCreateCsv.IsEnabled=$false
        $script:lblValidation.Text='Pass - imported saved data'
        $script:lblValidation.Foreground='LightGreen'
        Log "Explicit validation confirmed $($script:P2Rows.Count) row(s) loaded from saved Mobility Group data. Save Mobility Group Data remains disabled because the imported file is already the saved artifact." PASS
        return $true
    }
    $script:lblValidation.Text='No data source selected'
    $script:lblValidation.Foreground='Tomato'
    [Windows.MessageBox]::Show('Select Use Current VM List or Import Saved Data before validating.','Validate Configuration')|Out-Null
    return $false
}
function Update-MasterIncludeState {
    param([ValidateSet('VM','P2')][string]$Scope)
    if($script:MasterIncludeSync){return}
    $rows=if($Scope-eq'VM'){@($script:Rows)}else{@($script:P2Rows)}
    $control=if($Scope-eq'VM'){$script:chkMasterIncludeVMs}else{$script:chkMasterIncludeP2}
    if(-not$control){return}
    $script:MasterIncludeSync=$true
    try{
        if(-not$rows.Count){$control.IsChecked=$false}
        else{$included=@($rows|Where-Object Include).Count;if($included-eq0){$control.IsChecked=$false}elseif($included-eq$rows.Count){$control.IsChecked=$true}else{$control.IsChecked=$null}}
    }finally{$script:MasterIncludeSync=$false}
}
function Set-MasterIncludeState {
    param([ValidateSet('VM','P2')][string]$Scope,[bool]$Include)
    $rows=if($Scope-eq'VM'){@($script:Rows)}else{@($script:P2Rows)}
    foreach($row in $rows){$row.Include=$Include}
    if($Scope-eq'VM'){$script:gridVMs.Items.Refresh();Invalidate-Validation "Include All VMs set to $Include";Refresh-P2ScheduleRows}else{$script:gridP2.Items.Refresh();P2-Invalidate}
    Update-MasterIncludeState $Scope
    Log "Master Include [$Scope] set to $Include for $($rows.Count) row(s)." PASS
}
function Get-HcxTimeChoices {
    $times=[Collections.Generic.List[string]]::new();for($h=0;$h-lt24;$h++){foreach($m in 0,15,30,45){$times.Add(('{0:D2}:{1:D2}'-f$h,$m))}}
    return @($times)
}
function Get-HcxSchedulerCapabilities([string]$MigrationType){
    $isRav=$MigrationType -match '(?i)Replication-assisted vMotion|\bRAV\b|Bulk Migration|vSphereReplication'
    $isXvmotion=$MigrationType -match '(?i)HCX Assisted vMotion|xVMotion|HCX vMotion|^vMotion$'
    [pscustomobject]@{TransferScheduleSupported=$isRav;SwitchoverScheduleSupported=($isRav -or $isXvmotion);TransferModes=if($isRav){@('No Transfer Schedule','Set Transfer Schedule')}else{@('Not Applicable')};SwitchoverModes=if($isRav -or $isXvmotion){@('Start After Transfer','Set Switchover Schedule','Defer Switchover')}else{@('Start After Transfer')}}
}
function Refresh-P2ScheduleRows {
    $existing=@{};foreach($x in @($script:P2Schedules)){$existing[[int]$x.MobilityGroupNumber]=$x}
    $script:P2Schedules.Clear();$base=[string]$script:txtP2Name.Text;if([string]::IsNullOrWhiteSpace($base)){$base='HCX Mobility Group'}
    $suffix=1;$null=[int]::TryParse([string]$script:txtP2StartSuffix.Text,[ref]$suffix);$timeChoices=@(Get-HcxTimeChoices)
    $sourceRows=if(@($script:P2Rows|Where-Object Include).Count){@($script:P2Rows|Where-Object Include)}else{@($script:Rows|Where-Object Include)}
    foreach($g in @($sourceRows|Group-Object MobilityGroupNumber|Sort-Object{[int]$_.Name})){
        $n=[int]$g.Name;$migration=[string](@($g.Group|Select-Object -First 1).MigrationType);if(-not$migration){$migration=[string]$script:cmbMigrationType.Text}
        $cap=Get-HcxSchedulerCapabilities $migration;$old=$existing[$n]
        $script:P2Schedules.Add([pscustomobject]@{
            MobilityGroupNumber=$n;GeneratedName=('{0}-{1:d2}'-f$base,($suffix+$n-1));MigrationType=$migration
            TransferScheduleSupported=[bool]$cap.TransferScheduleSupported;TransferModes=@($cap.TransferModes);TransferMode=if($old -and $cap.TransferModes-contains$old.TransferMode){$old.TransferMode}else{[string]$cap.TransferModes[0]}
            TransferStartDate=if($old){$old.TransferStartDate}else{$null};TransferStartTime=if($old){$old.TransferStartTime}else{'00:00'};TransferExpiryDate=if($old){$old.TransferExpiryDate}else{$null};TransferExpiryTime=if($old){$old.TransferExpiryTime}else{'00:00'}
            SwitchoverModes=@($cap.SwitchoverModes);SwitchoverMode=if($old -and $cap.SwitchoverModes-contains$old.SwitchoverMode){$old.SwitchoverMode}else{'Start After Transfer'}
            SwitchoverStartDate=if($old){$old.SwitchoverStartDate}else{$null};SwitchoverStartTime=if($old){$old.SwitchoverStartTime}else{'00:00'};SwitchoverEndDate=if($old){$old.SwitchoverEndDate}else{$null};SwitchoverEndTime=if($old){$old.SwitchoverEndTime}else{'00:00'}
            TimeChoices=$timeChoices;TransferStartEpochMs=if($old){$old.TransferStartEpochMs}else{$null};TransferExpiryEpochMs=if($old){$old.TransferExpiryEpochMs}else{$null};SwitchoverStartEpochMs=if($old){$old.SwitchoverStartEpochMs}else{$null};SwitchoverEndEpochMs=if($old){$old.SwitchoverEndEpochMs}else{$null};Validation=if($old){$old.Validation}else{'Not validated'}
        })
    }
    $script:gridP2Schedules.ItemsSource=$script:P2Schedules;$script:gridP2Schedules.Items.Refresh();if($script:P2Schedules.Count){if(@($script:P2Schedules|Where-Object Validation -ne 'Pass').Count){Set-SchedulerStatus "$($script:P2Schedules.Count) group(s) loaded - not validated" '#76C7D8'}else{Set-SchedulerStatus 'Pass' 'LightGreen'}}else{Set-SchedulerStatus 'No included VM groups found' 'Gold'}
}
function Convert-HcxLocalScheduleTime($Date,[string]$Time,[string]$Label){
    if($null -eq $Date -or [string]::IsNullOrWhiteSpace($Time)){throw "$Label requires both a calendar date and time."}
    $match=[regex]::Match($Time.Trim(),'^(?<hour>[01]?[0-9]|2[0-3]):(?<minute>[0-5][0-9])$')
    if(-not$match.Success){throw "$Label time '$Time' is invalid. Select a time in HH:mm format."}
    $hour=[int]$match.Groups['hour'].Value;$minute=[int]$match.Groups['minute'].Value
    $local=([datetime]$Date).Date.AddHours($hour).AddMinutes($minute)
    return [datetimeoffset]::new($local).ToUnixTimeMilliseconds()
}
function Set-SchedulerStatus([string]$Text,[string]$Color){
    if($script:lblSchedulerStatus){$script:lblSchedulerStatus.Text=$Text;$script:lblSchedulerStatus.Foreground=$Color}
    if($script:lblSchedulerStatusTop){$script:lblSchedulerStatusTop.Text=$Text;$script:lblSchedulerStatusTop.Foreground=$Color}
}
function Validate-P2Schedules {
    Commit-Grid -Grid $script:gridP2Schedules;$errors=[Collections.Generic.List[string]]::new()
    foreach($row in @($script:P2Schedules)){
        try{
            $row.TransferStartEpochMs=$null;$row.TransferExpiryEpochMs=$null;$row.SwitchoverStartEpochMs=$null;$row.SwitchoverEndEpochMs=$null
            if($row.TransferScheduleSupported -and $row.TransferMode-eq'Set Transfer Schedule'){$row.TransferStartEpochMs=Convert-HcxLocalScheduleTime $row.TransferStartDate $row.TransferStartTime "Group $($row.MobilityGroupNumber) Transfer Start";$row.TransferExpiryEpochMs=Convert-HcxLocalScheduleTime $row.TransferExpiryDate $row.TransferExpiryTime "Group $($row.MobilityGroupNumber) Transfer Expiry";if($row.TransferExpiryEpochMs -le $row.TransferStartEpochMs){throw 'Transfer Expiry must be later than Transfer Start.'}}
            if($row.SwitchoverMode-eq'Set Switchover Schedule'){$row.SwitchoverStartEpochMs=Convert-HcxLocalScheduleTime $row.SwitchoverStartDate $row.SwitchoverStartTime "Group $($row.MobilityGroupNumber) Switchover Start";$row.SwitchoverEndEpochMs=Convert-HcxLocalScheduleTime $row.SwitchoverEndDate $row.SwitchoverEndTime "Group $($row.MobilityGroupNumber) Switchover End";if($row.SwitchoverEndEpochMs -le $row.SwitchoverStartEpochMs){throw 'Switchover End must be later than Switchover Start.'}}
            $row.Validation='Pass'
        }catch{$row.Validation="Failed - $($_.Exception.Message)";$errors.Add("Group $($row.MobilityGroupNumber): $($_.Exception.Message)")}
    }
    $script:gridP2Schedules.Items.Refresh();if($errors.Count){Set-SchedulerStatus "Failed ($($errors.Count))" 'Tomato';foreach($e in $errors){ Log $e ERROR };[Windows.MessageBox]::Show(($errors-join"`n"),'Schedule Validation')|Out-Null;return $false};Set-SchedulerStatus 'Pass' 'LightGreen';Log "Scheduler validation passed for $($script:P2Schedules.Count) Mobility Group(s)." PASS;return $true
}
function Get-P2GroupSchedules([int]$GroupNumber){
    if(@($script:P2Schedules|Where-Object Validation -ne 'Pass').Count){Log 'Automatic pre-payload Scheduler validation started.' INFO;if(-not(Validate-P2Schedules)){throw 'One or more Mobility Group schedules failed validation.'}}
    $row=@($script:P2Schedules|Where-Object{[int]$_.MobilityGroupNumber-eq$GroupNumber}|Select-Object -First 1);if(-not$row.Count){return [pscustomobject]@{Transfer=[ordered]@{};Switchover=[ordered]@{}}};$r=$row[0]
    if($r.Validation-ne'Pass'){throw "Mobility Group $GroupNumber schedule is not validated."}
    $transfer=if($r.TransferScheduleSupported -and $r.TransferMode-eq'Set Transfer Schedule'){[ordered]@{startTime=[int64]$r.TransferStartEpochMs;expiryTime=[int64]$r.TransferExpiryEpochMs}}else{[ordered]@{}}
    $switchover=if($r.SwitchoverMode-eq'Set Switchover Schedule'){[ordered]@{startTime=[int64]$r.SwitchoverStartEpochMs;expiryTime=[int64]$r.SwitchoverEndEpochMs}}else{[ordered]@{}}
    [pscustomobject]@{Transfer=$transfer;Switchover=$switchover;SwitchoverMode=$r.SwitchoverMode}
}
function ConvertTo-HcxScheduleDate([string]$Value,[string]$Label){
 if([string]::IsNullOrWhiteSpace($Value)){return $null};$d=[datetime]::MinValue
 foreach($f in @('yyyy-MM-dd','M/d/yyyy','MM/dd/yyyy')){if([datetime]::TryParseExact($Value.Trim(),$f,[Globalization.CultureInfo]::InvariantCulture,[Globalization.DateTimeStyles]::None,[ref]$d)){return $d.Date}}
 throw "$Label date '$Value' is invalid. Use yyyy-MM-dd or M/d/yyyy."
}
function ConvertTo-HcxScheduleTime([string]$Value,[string]$Label){
 if([string]::IsNullOrWhiteSpace($Value)){return ''};$m=[regex]::Match($Value.Trim(),'^(?<h>[01]?[0-9]|2[0-3]):(?<m>[0-5][0-9])$');if(-not$m.Success){throw "$Label time '$Value' is invalid. Use HH:mm."};return ('{0:D2}:{1:D2}' -f [int]$m.Groups['h'].Value,[int]$m.Groups['m'].Value)
}
function Save-ExampleScheduleCsv {
    $path=Join-Path $script:OutputBase 'HCX91-Scheduler-Import-Example.csv'
    @(
      [pscustomobject][ordered]@{MobilityGroupNumber=1;TransferMode='No Transfer Schedule';TransferStartDate='';TransferStartTime='';TransferExpiryDate='';TransferExpiryTime='';SwitchoverMode='Set Switchover Schedule';SwitchoverStartDate=(Get-Date).AddDays(1).ToString('yyyy-MM-dd');SwitchoverStartTime='21:00';SwitchoverEndDate=(Get-Date).AddDays(2).ToString('yyyy-MM-dd');SwitchoverEndTime='01:00'}
      [pscustomobject][ordered]@{MobilityGroupNumber=2;TransferMode='Set Transfer Schedule';TransferStartDate=(Get-Date).AddDays(1).ToString('yyyy-MM-dd');TransferStartTime='18:00';TransferExpiryDate=(Get-Date).AddDays(2).ToString('yyyy-MM-dd');TransferExpiryTime='06:00';SwitchoverMode='Set Switchover Schedule';SwitchoverStartDate=(Get-Date).AddDays(2).ToString('yyyy-MM-dd');SwitchoverStartTime='21:00';SwitchoverEndDate=(Get-Date).AddDays(3).ToString('yyyy-MM-dd');SwitchoverEndTime='01:00'}
    )|Export-Csv -LiteralPath $path -NoTypeInformation -Encoding utf8BOM
    Log "Example Scheduler CSV saved: $path" PASS;return $path
}
function Import-P2ScheduleCsv {
    $dlg=[Microsoft.Win32.OpenFileDialog]::new();$dlg.Filter='CSV files (*.csv)|*.csv|All files (*.*)|*.*';$dlg.Title='Import Mobility Group Scheduler CSV';if(-not$dlg.ShowDialog()){return}
    $data=@(Import-Csv -LiteralPath $dlg.FileName);if(-not$data.Count){throw 'Scheduler CSV contains no rows.'}
    $required=@('MobilityGroupNumber','TransferMode','TransferStartDate','TransferStartTime','TransferExpiryDate','TransferExpiryTime','SwitchoverMode','SwitchoverStartDate','SwitchoverStartTime','SwitchoverEndDate','SwitchoverEndTime')
    foreach($h in $required){if(-not$data[0].PSObject.Properties[$h]){throw "Scheduler CSV is missing required column '$h'."}}
    $seen=@{};foreach($item in $data){$n=0;if(-not[int]::TryParse([string]$item.MobilityGroupNumber,[ref]$n)-or$n-lt1){throw "Invalid MobilityGroupNumber '$($item.MobilityGroupNumber)'."};if($seen.ContainsKey($n)){throw "Duplicate MobilityGroupNumber '$n' in Scheduler CSV."};$seen[$n]=$true
      $row=@($script:P2Schedules|Where-Object{[int]$_.MobilityGroupNumber-eq$n}|Select-Object -First 1);if(-not$row.Count){throw "Scheduler CSV references group $n, but that group is not present in the current workload."};$r=$row[0]
      if($r.TransferScheduleSupported){$incomingTransferMode=if([string]$item.TransferMode -eq 'Start Now'){'No Transfer Schedule'}else{[string]$item.TransferMode};if($incomingTransferMode -and ($r.TransferModes -contains $incomingTransferMode)){$r.TransferMode=$incomingTransferMode}}else{$r.TransferMode='Not Applicable';$r.TransferStartDate=$null;$r.TransferStartTime='00:00';$r.TransferExpiryDate=$null;$r.TransferExpiryTime='00:00';if($item.TransferMode -or $item.TransferStartDate -or $item.TransferStartTime -or $item.TransferExpiryDate -or $item.TransferExpiryTime){Log "Group $n transfer fields were ignored because '$($r.MigrationType)' does not support transfer scheduling." WARN}}
      if($item.SwitchoverMode){if(-not($r.SwitchoverModes -contains $item.SwitchoverMode)){throw "Group $n SwitchoverMode '$($item.SwitchoverMode)' is not valid for migration type '$($r.MigrationType)'."};$r.SwitchoverMode=$item.SwitchoverMode}
      foreach($pair in @(@('SwitchoverStartDate','SwitchoverStartDate'),@('SwitchoverEndDate','SwitchoverEndDate'))){$value=[string]$item.($pair[0]);if($value){$dt=[datetime]::MinValue;$r.($pair[1])=ConvertTo-HcxScheduleDate $value "Group $n $($pair[0])"}}
      foreach($name in @('SwitchoverStartTime','SwitchoverEndTime')){if([string]$item.$name){$r.$name=ConvertTo-HcxScheduleTime ([string]$item.$name) "Group $n $name"}}
      $r.Validation='Not validated'
    }
    $script:gridP2Schedules.Items.Refresh();Set-SchedulerStatus 'Imported - validation required' '#76C7D8';P2-Invalidate;Log "Imported Scheduler CSV: $($dlg.FileName); Rows=$($data.Count)." PASS
}
function P2-Import{
 ...
 try{
  ...
  foreach($r in @($csv)){
   try{
    ...
    $newRows.Add([pscustomobject]@{Include=$true;VMName=$r.VMName;VMId=$r.VMId;MigrationType=if($r.MigrationType){[string]$r.MigrationType}else{'HCX Assisted vMotion (Direct)'};Compute=$r.DestinationCompute;ComputeId=$r.DestinationComputeId;ComputeType=if($r.DestinationComputeType){$r.DestinationComputeType}else{'cluster'};MobilityGroupNumber=if($r.MobilityGroupNumber){[int]$r.MobilityGroupNumber}else{1};Folder=$r.DestinationFolder;FolderId=$r.DestinationFolderId;StorageType=if($r.DestinationStorageType){$r.DestinationStorageType}else{'datastore'};Storage=$r.DestinationDatastore;StorageId=$r.DestinationDatastoreId;StoragePolicy=if($r.DestinationStoragePolicy){[string]$r.DestinationStoragePolicy}else{''};Vtpm=if(([string]$r.vTPM).Trim()-eq'1'){1}else{0};StoragePolicyAssignment=if($r.StoragePolicyAssignment){$r.StoragePolicyAssignment}else{'Standard Global'};StorageSelectionSource='Phase I Import';DiskFormat=if($r.DiskFormat){[string]$r.DiskFormat}else{'Same format as source'};DiskFormatSelectionSource='Phase I Import';Networks=@($net);NetworkSummary=(@($net|ForEach-Object{"$($_.SourceNetworkName) -> $($_.DestinationNetworkName)"})-join'; ')})
  ...
  Resolve-P2ImportedNetworks;P2-Invalidate;Update-MasterIncludeState P2;Refresh-P2ScheduleRows
  $script:ValidationCurrent=$false;$script:btnCreateCsv.IsEnabled=$false;$script:lblValidation.Text='Not validated - select Validate Configuration';$script:lblValidation.Foreground='#76C7D8'
...
function P2-Inventory{
 ...
 $script:cmbP2Policy.Items.Clear();$script:cmbP2VtpmPolicy.Items.Clear();foreach($x in @($script:Inventory.Policies|Sort-Object Name)){[void]$script:cmbP2Policy.Items.Add($x);[void]$script:cmbP2VtpmPolicy.Items.Add($x)}
...
function Convert-HcxMigrationTypeDisplay([string]$Type,[string]$Name){
    $token=if($Type){$Type}else{$Name}
    switch -Regex ($token) {
      '^(?i:RAV)$|Replication-assisted' { return 'Replication-assisted vMotion' }
      '^(?i:vMotion)$' { return 'HCX vMotion' }
      '^(?i:xVMotion|nativeVMotion)$|HCX Assisted' { return 'HCX Assisted vMotion (Direct)' }
      '^(?i:vSphereReplication)$|Bulk' { return 'Bulk Migration' }
      '^(?i:ColdMigration)$|Cold Migration' { return 'Cold Migration' }
      '^(?i:OsAssistedMigration)$|OS Assisted' { return 'OS Assisted Migration' }
      default { return $Name }
    }
}
function Set-HcxSupportedMigrationTypeInventory($Response){
    $items=@(Get-HcxResponseItems $Response);$names=[Collections.Generic.List[string]]::new()
    function Add-HcxMigrationTypeCandidate($Item){
      if($null-eq$Item){return};$type=[string](Get-HcxFirstValue $Item @('migrationType','type','id','value'));$name=[string](Get-HcxFirstValue $Item @('displayName','name','label'))
      $display=Convert-HcxMigrationTypeDisplay $type $name;if($display-and-not$names.Contains($display)){$names.Add($display)}
      foreach($prop in @($Item.PSObject.Properties)){if($prop.Value -is [Collections.IEnumerable] -and $prop.Value -isnot [string]){foreach($child in @($prop.Value)){if($child -isnot [string]){Add-HcxMigrationTypeCandidate $child}}}}
    }
    foreach($item in $items){Add-HcxMigrationTypeCandidate $item}
    if(-not$names.Count){foreach($fallback in @('Replication-assisted vMotion','HCX vMotion','HCX Assisted vMotion (Direct)','Bulk Migration','Cold Migration','OS Assisted Migration')){$names.Add($fallback)};Log 'Supported migration-type response could not be normalized; complete HCX 9.1 fallback list applied.' WARN}
    $script:MigrationTypes=@($names);Set-HcxBindingContextCollection 'MigrationTypes' $script:MigrationTypes;Set-HcxItemsSource $script:cmbMigrationType $script:MigrationTypes
    if($script:MigrationTypes -contains 'HCX Assisted vMotion (Direct)'){$script:cmbMigrationType.SelectedItem='HCX Assisted vMotion (Direct)'}elseif($script:MigrationTypes.Count){$script:cmbMigrationType.SelectedIndex=0}
    Log "HCX migration types loaded: $($script:MigrationTypes -join ', ')." PASS
}
...
function Initialize-HcxAutomaticDiscovery {
    ...
    try {
        ...
        Set-HcxSupportedMigrationTypeInventory $supported
...
function P2-Build{
 ...
 $policyDescriptor=Get-P2StoragePolicyDescriptor ([string]$first.StoragePolicy)
 if(-not$policyDescriptor){throw "Unable to resolve storage policy descriptor for '$($first.StoragePolicy)'."}
 $policyDescriptors=@{};foreach($policyName in @($included|ForEach-Object{[string]$_.StoragePolicy}|Where-Object{$_}|Select-Object -Unique)){$resolvedPolicy=Get-P2StoragePolicyDescriptor $policyName;if(-not$resolvedPolicy){throw "Unable to resolve storage policy descriptor for '$policyName'."};$policyDescriptors[$policyName]=$resolvedPolicy}
 Log "Phase II group storage policy: Name='$($policyDescriptor.Name)'; Value='$($policyDescriptor.Value)'; Type='$($policyDescriptor.Type)'; Source='$($policyDescriptor.Source)'." PASS
 ...
 $groupSchedule=Get-P2GroupSchedules ([int]$script:CurrentBuildingGroupNumber)
 $effectiveMigration=[string]$first.MigrationType;$isRav=$effectiveMigration -match '(?i)Replication-assisted vMotion|\bRAV\b';$payloadMigrationType=switch -Regex($effectiveMigration){'(?i)^HCX vMotion$|^vMotion$'{'vMotion'};'(?i)HCX Assisted|xVMotion|nativeVMotion'{'xVMotion'};'(?i)Replication-assisted|^RAV$'{'RAV'};'(?i)Bulk|vSphereReplication'{'vSphereReplication'};'(?i)Cold'{'ColdMigration'};'(?i)OS Assisted|OsAssistedMigration'{'OsAssistedMigration'};default{'xVMotion'}};$isReplication=$payloadMigrationType -in @('RAV','vSphereReplication')
 $transferProfile=[System.Collections.Generic.List[object]]::new()
 if($isReplication){
  [void]$transferProfile.Add([ordered]@{option='removeSnapshots';value=$false})
  [void]$transferProfile.Add([ordered]@{option='removeISOs';value=[bool]$script:chkP2Iso.IsChecked})
  [void]$transferProfile.Add([ordered]@{option='longRecovery';value=$false})
 }else{
  [void]$transferProfile.Add([ordered]@{option='removeISOs';value=[bool]$script:chkP2Iso.IsChecked})
 }
 $groupDefaults=[ordered]@{
  source=$source;destination=$destination;migrationType=$payloadMigrationType;servicemeshId=$serviceMeshId
  transferParams=[ordered]@{schedule=$groupSchedule.Transfer;transferType=if($isReplication){'vSphereReplication'}else{'NO_OP'};transferProfile=$transferProfile}
  switchoverParams=[ordered]@{
   switchoverType=$payloadMigrationType;schedule=$groupSchedule.Switchover
   switchoverProfile=@(
  ...
  storage=[ordered]@{defaultStorage=[ordered]@{
   ...
   storageParams=@([ordered]@{option='StorageProfile';value=[string]$policyDescriptor.Value;type=[string]$policyDescriptor.Type;name=[string]$policyDescriptor.Name})
 ...
 foreach($row in $included){
  ...
  $migrationIntents+=,[ordered]@{
   ...
   storage=if([int]$row.Vtpm -eq 1 -or [string]$row.StoragePolicyAssignment -eq 'Per-VM Override'){[ordered]@{defaultStorage=[ordered]@{id=(Convert-P2MoRef ([string]$row.StorageId));name=[string]$row.Storage;type=if($row.StorageType-match'storagepod|Cluster'){'storagepod'}else{'datastore'};storageParams=@([ordered]@{option='StorageProfile';value=[string]$policyDescriptors[[string]$row.StoragePolicy].Value;type=[string]$policyDescriptors[[string]$row.StoragePolicy].Type;name=[string]$policyDescriptors[[string]$row.StoragePolicy].Name});diskProvisionType=(Convert-P2DiskFormat ([string]$row.DiskFormat))}}}else{$null}
...
function P2-BuildAll{
 if([string]$script:txtP2Csv.Text -like 'Current*'){$null=Test-Phase1StoragePolicyConsistency -Rows $script:Rows -ThrowOnError}
 $base=$script:txtP2Name.Text.Trim();$count=[int]$script:cmbP2GroupCount.SelectedItem
 $startSuffix=0
 if(-not[int]::TryParse([string]$script:txtP2StartSuffix.Text,[ref]$startSuffix)-or$startSuffix-lt1-or$startSuffix-gt9999){throw 'Starting Suffix must be a whole number from 1 through 9999.'}
 if(-not$base){throw 'Base Group Name is required.'};if($count-lt1-or$count-gt 50){throw 'Number of groups must be 1 through 50.'}
 $lastSuffix=$startSuffix+$count-1;if($lastSuffix-gt9999){throw "The requested group range ends at suffix $lastSuffix. Starting Suffix plus group count must not exceed 9999."}
 ...
 try{
  $script:SuppressScheduleRefresh=$true
  foreach($n in 1..$count){
   $script:CurrentBuildingGroupNumber=$n
   $members=@($script:P2Rows|Where-Object{$state[$_.VMName]-and[int]$_.MobilityGroupNumber-eq$n});if(-not$(Get-SafeCount $members)){throw "Mobility Group $n has no included VMs."}
   foreach($r in $script:P2Rows){$r.Include=($state[$r.VMName]-and[int]$r.MobilityGroupNumber-eq$n)}
   $suffix=$startSuffix+$n-1
   $script:txtP2Name.Text='{0}-{1:d2}'-f$base,$suffix
   Log "Building Mobility Group source group $n as '$($script:txtP2Name.Text)' using starting suffix $startSuffix." INFO
   $payload=P2-Build;$payload=script:Normalize-P2HostAndStoragePodPayload -Payload $payload
   $list.Add([pscustomobject]@{Number=$n;Suffix=$suffix;Name=$script:txtP2Name.Text;VMCount=$(Get-SafeCount $members);Payload=$payload})
  }
 }
 finally{$script:SuppressScheduleRefresh=$false;$script:CurrentBuildingGroupNumber=0;foreach($r in $script:P2Rows){$r.Include=$state[$r.VMName]};$script:txtP2Name.Text=$originalName;$script:gridP2.Items.Refresh()}
 return @($list)
}
function P2-Validate {
    ...
    if(-not$(@($script:P2Rows).Count)){$errors.Add('Select Use Current VM List or Import Saved Data.')}elseif(-not(Validate-P2Schedules)){$errors.Add('One or more Mobility Group schedules failed validation.')}
    if([string]$script:txtP2Csv.Text -like 'Current*'){try{$null=Test-Phase1StoragePolicyConsistency -Rows $script:Rows -ThrowOnError}catch{$errors.Add($_.Exception.Message)}}
    if([string]::IsNullOrWhiteSpace($script:txtP2Name.Text)){$errors.Add('Base Group Name is required.')};$gc=[int]$script:cmbP2GroupCount.SelectedItem;$startSuffix=0;if(-not[int]::TryParse([string]$script:txtP2StartSuffix.Text,[ref]$startSuffix)-or$startSuffix-lt1-or$startSuffix-gt9999){$errors.Add('Starting Suffix must be a whole number from 1 through 9999.')}elseif(($startSuffix+$gc-1)-gt9999){$errors.Add('Starting Suffix plus group count must not exceed 9999.')};foreach($n in 1..$gc){if(-not@($script:P2Rows|Where-Object{$_.Include-and[int]$_.MobilityGroupNumber-eq$n}).Count){$errors.Add("Mobility Group $n has no included VMs.")}}
...
function P2-Preview{try{$null=Assert-HcxAutomaticDiscoveryCurrent;if(-not(Validate-P2Schedules)){throw 'One or more Mobility Group schedules failed validation.'};$script:Window.Cursor='Wait';$script:P2Payload=@(P2-BuildAll);$folder=script:Get-P2ArtifactFolder;foreach($x in $script:P2Payload){$path=Join-Path $folder("HCX91-$($x.Name)-Payload.json");$x.Payload|ConvertTo-Json -Depth 50|Set-Content $path -Encoding utf8BOM};Invoke-Item $folder}finally{$script:Window.Cursor=$null}}
...
function Assert-HcxMobilityGroupPayloadCollections($Payload){
    foreach($item in @($Payload.items)){
        $tp=$item.groupDefaults.transferParams.transferProfile
        if($null-eq$tp){throw "Mobility Group '$($item.name)' transferProfile is missing."}
        if($tp -is [Collections.IDictionary] -or $tp -is [Management.Automation.PSCustomObject]){throw "Mobility Group '$($item.name)' transferProfile must be a JSON array, but the current value is a single object."}
        if(@($tp).Count-lt1){throw "Mobility Group '$($item.name)' transferProfile is empty."}
        $sp=$item.groupDefaults.switchoverParams.switchoverProfile
        if($sp -is [Collections.IDictionary] -or $sp -is [Management.Automation.PSCustomObject]){throw "Mobility Group '$($item.name)' switchoverProfile must be a JSON array."}
    }
    Log 'Payload collection contract passed: transferProfile and switchoverProfile are arrays.' PASS
}
function Submit-HcxMobilityGroup {
    param([Parameter(Mandatory=$true)][string]$GroupName,[Parameter(Mandatory=$true)]$Payload)
    Assert-HcxMobilityGroupPayloadCollections $Payload
...
function P2-Create{
 ...
 $null=Assert-HcxAutomaticDiscoveryCurrent
 if(-not(Validate-P2Schedules)){throw 'One or more Mobility Group schedules failed validation.'};if(-not$script:P2Payload){$script:P2Payload=@(P2-BuildAll)}
 $summary=@($script:P2Payload|ForEach-Object{"$($_.Name): $($_.VMCount) VM(s)"})-join[Environment]::NewLine
 if([Windows.MessageBox]::Show("Create these Mobility Group drafts in HCX?`n`n$summary",'Create Mobility Groups','YesNo')-ne'Yes'){return}
 ...
 $script:lblP2Status.Text="$ok draft(s) saved";Log "Mobility Group creation completed successfully: $ok draft(s) saved." PASS;Show-HcxThemedNotice -Title 'HCX Mobility Group Drafts' -Heading 'Mobility Group creation completed' -Kind Success -Message "Created $ok HCX draft Mobility Group(s). No migration was started." -Details ((@($script:P2Payload|ForEach-Object{"$($_.Name)  |  $($_.VMCount) VM(s)"})) -join "`n")
}
...
function script:Normalize-P2HostAndStoragePodPayload {
    ...
    if([string]::IsNullOrWhiteSpace($folder) -or -not(Test-Path -LiteralPath $folder -PathType Container)){
        if(Get-Command -Name 'script:Get-P2ArtifactFolder' -ErrorAction SilentlyContinue){$folder=[string](script:Get-P2ArtifactFolder)}
    }
...
<Grid><Grid.ColumnDefinitions><ColumnDefinition Width="500" MinWidth="500"/><ColumnDefinition Width="*" MinWidth="720"/></Grid.ColumnDefinitions>
<GroupBox Header="Prerequisites"><Grid><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="*"/><RowDefinition Height="Auto"/></Grid.RowDefinitions><Grid Margin="4,2,4,4">
...
<Grid.RowDefinitions><RowDefinition Height="25"/><RowDefinition Height="25"/><RowDefinition Height="25"/><RowDefinition Height="25"/></Grid.RowDefinitions>
...
<StackPanel Grid.Row="1" Grid.Column="0" Grid.ColumnSpan="3" Orientation="Horizontal" HorizontalAlignment="Left"><TextBlock Text="HCX API:" MinWidth="125"/><TextBlock x:Name="lblApi"/></StackPanel><StackPanel Grid.Row="2" Grid.Column="0" Grid.ColumnSpan="3" Orientation="Horizontal" HorizontalAlignment="Left"><TextBlock Text="Source vCenter:" MinWidth="125"/><TextBlock x:Name="lblSourceVCStatus" Text="Not connected" Foreground="#76C7D8"/></StackPanel><StackPanel Grid.Row="3" Grid.Column="0" Grid.ColumnSpan="3" Orientation="Horizontal" HorizontalAlignment="Left"><TextBlock Text="Destination vCenter:" MinWidth="125"/><TextBlock x:Name="lblDestinationVCStatus" Text="Not connected" Foreground="#76C7D8"/></StackPanel>
<StackPanel Grid.Row="1" Grid.Column="2" Orientation="Horizontal" HorizontalAlignment="Center"></StackPanel>
</Grid>
<StackPanel Grid.Row="2" Orientation="Horizontal" HorizontalAlignment="Center"><Button x:Name="btnRecheck" Content="Recheck"/><Button x:Name="btnInstall" Content="Install VCF.PowerCLI"/></StackPanel></Grid></GroupBox>
...
<TabControl x:Name="tabWorkflow" Grid.Row="1" Grid.RowSpan="4" Margin="0,6,0,0">
<TabItem Header="1. VM Import"><Grid Margin="8"><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="*"/><RowDefinition Height="Auto"/></Grid.RowDefinitions>
<GroupBox Header="VM Inventory Import and Inclusion"><WrapPanel><Button x:Name="btnImport" Content="Import VM CSV"/><Button x:Name="btnExample" Content="Download Example VM CSV"/><Button x:Name="btnSelectAllVMs" Content="Select All VMs"/><Button x:Name="btnUnselectAllVMs" Content="Unselect All VMs"/><Button x:Name="btnRemove" Content="Remove Selected VM(s)"/><Button x:Name="btnClear" Content="Clear Grid"/><Button x:Name="btnRefresh" Content="Refresh Discovery"/><TextBlock Text="Inventory-only view. Configuration is completed on the Network, Storage, and Destination tabs." Foreground="#76C7D8" VerticalAlignment="Center" Margin="12,3"/></WrapPanel></GroupBox>
<Border Grid.Row="1" VerticalAlignment="Top" Panel.ZIndex="10" Background="#0D1B22" BorderBrush="#607D8B" BorderThickness="0,0,0,1" Padding="5"><StackPanel Orientation="Horizontal"><CheckBox x:Name="chkMasterIncludeVMs" Content="Include All VMs" IsThreeState="True" IsChecked="False" ToolTip="Checked includes all VMs. Cleared excludes all VMs. Indeterminate means only some VMs are included." FontWeight="SemiBold" Margin="5,1,10,1"/><TextBlock Text="Controls the Include check boxes for the complete VM list below." Foreground="#76C7D8" VerticalAlignment="Center"/></StackPanel></Border>
<DataGrid x:Name="gridVMs" Grid.Row="1" Margin="0,38,0,0" AutoGenerateColumns="False" CanUserAddRows="False" SelectionMode="Extended" VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Auto" EnableRowVirtualization="True" EnableColumnVirtualization="True" ScrollViewer.CanContentScroll="True"><DataGrid.RowStyle><Style TargetType="DataGridRow"><Style.Triggers><DataTrigger Binding="{Binding Status}" Value="NotFound"><Setter Property="Background" Value="#8B1E1E"/><Setter Property="Foreground" Value="White"/><Setter Property="ToolTip" Value="VM was not found in source vCenter. Uncheck Include or remove this row."/></DataTrigger><DataTrigger Binding="{Binding PowerState}" Value="PoweredOff"><Setter Property="Background" Value="#8B1E1E"/><Setter Property="Foreground" Value="White"/><Setter Property="ToolTip" Value="VM is powered off. Review, uncheck Include, or remove this row."/></DataTrigger></Style.Triggers></Style></DataGrid.RowStyle><DataGrid.Columns>
<DataGridCheckBoxColumn Header="Include" Binding="{Binding Include, Mode=TwoWay}" Width="60"/><DataGridTextColumn Header="Group #" Binding="{Binding MobilityGroupNumber, Mode=TwoWay}" Width="70"/><DataGridTextColumn Header="Inventory" Binding="{Binding Status}" IsReadOnly="True" Width="90"/><DataGridTextColumn Header="VM Name" Binding="{Binding VMName}" IsReadOnly="True" Width="170"/><DataGridTextColumn Header="Power" Binding="{Binding PowerState}" IsReadOnly="True" Width="90"/><DataGridTextColumn Header="Guest OS" Binding="{Binding GuestOS}" IsReadOnly="True" Width="230"/><DataGridTextColumn Header="CPU" Binding="{Binding CPU}" IsReadOnly="True" Width="55"/><DataGridTextColumn Header="Memory GB" Binding="{Binding MemoryGB}" IsReadOnly="True" Width="85"/><DataGridTextColumn Header="vTPM" Binding="{Binding Vtpm}" IsReadOnly="True" Width="60"/><DataGridTextColumn Header="vTPM Detection" Binding="{Binding VtpmDetectionStatus}" IsReadOnly="True" Width="120"/><DataGridTextColumn Header="Source Compute" Binding="{Binding SourceCompute}" IsReadOnly="True" Width="170"/><DataGridTextColumn Header="Source Datastore" Binding="{Binding SourceDatastore}" IsReadOnly="True" Width="180"/><DataGridTextColumn Header="Source Networks" Binding="{Binding SourceNetworks}" IsReadOnly="True" Width="230"/><DataGridTextColumn Header="NICs" Binding="{Binding NicCount}" IsReadOnly="True" Width="50"/><DataGridTextColumn Header="Finding / Validation" Binding="{Binding ValidationMessage}" IsReadOnly="True" Width="300"/>
</DataGrid.Columns></DataGrid>
<TextBlock Grid.Row="2" Text="Every import automatically writes the complete inventory findings CSV to the active run folder." Foreground="#76C7D8" Margin="5"/></Grid></TabItem>

<TabItem Header="2. Network Mappings"><Grid Margin="8"><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="*"/></Grid.RowDefinitions>
<GroupBox Header="Optional Bulk Network Mapping"><Grid><Grid.ColumnDefinitions><ColumnDefinition Width="130"/><ColumnDefinition Width="*"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="160"/></Grid.ColumnDefinitions><TextBlock Text="Network Mapping CSV" VerticalAlignment="Center"/><TextBox x:Name="txtNetworkMappingCsv" Grid.Column="1" IsReadOnly="True" ToolTip="SourceNetworkName,DestinationNetworkName"/><Button x:Name="btnImportNetworkMapping" Grid.Column="2" Content="Import Mapping CSV"/><Button x:Name="btnClearNetworkMapping" Grid.Column="3" Content="Clear Mapping CSV"/><StackPanel Grid.Column="4" Orientation="Horizontal"><TextBlock Text="Status:"/><TextBlock x:Name="lblNetworkMappingStatus" Text="Not loaded" Foreground="#76C7D8"/></StackPanel></Grid></GroupBox>
<DataGrid x:Name="gridNetworkRows" Grid.Row="1" ItemsSource="{Binding ElementName=gridVMs, Path=ItemsSource}" AutoGenerateColumns="False" CanUserAddRows="False" IsReadOnly="True" VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Auto"><DataGrid.Columns><DataGridCheckBoxColumn Header="Include" Binding="{Binding Include}" Width="60"/><DataGridTextColumn Header="Group #" Binding="{Binding MobilityGroupNumber}" Width="70"/><DataGridTextColumn Header="VM Name" Binding="{Binding VMName}" Width="170"/><DataGridTextColumn Header="Source Networks" Binding="{Binding SourceNetworks}" Width="260"/><DataGridTextColumn Header="NIC Count" Binding="{Binding NicCount}" Width="75"/><DataGridTextColumn Header="Mapping Summary" Binding="{Binding NicSummary}" Width="*"/><DataGridTemplateColumn Header="Action" Width="100"><DataGridTemplateColumn.CellTemplate><DataTemplate><Button Content="Configure" Tag="{Binding}" Padding="5,2"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn></DataGrid.Columns></DataGrid>
</Grid></TabItem>

<TabItem Header="3. Destination"><Grid Margin="8"><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="*"/><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/></Grid.RowDefinitions>
<GroupBox Header="Destination and Migration Defaults"><Grid><Grid.ColumnDefinitions><ColumnDefinition Width="Auto"/><ColumnDefinition Width="210"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="240"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="230"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="240"/></Grid.ColumnDefinitions><TextBlock Text="Destination Site"/><ComboBox x:Name="cmbDestinationSite" Grid.Column="1" DisplayMemberPath="Name"/><TextBlock Grid.Column="2" Text="Compute"/><ComboBox x:Name="cmbGlobalCompute" Grid.Column="3" DisplayMemberPath="DisplayName"/><TextBlock Grid.Column="4" Text="VM Folder"/><ComboBox x:Name="cmbGlobalFolder" Grid.Column="5" DisplayMemberPath="Name"/><TextBlock Grid.Column="6" Text="Migration Type"/><ComboBox x:Name="cmbMigrationType" Grid.Column="7" SelectedIndex="0"/></Grid></GroupBox>
<DataGrid x:Name="gridDestinationRows" Grid.Row="1" ItemsSource="{Binding ElementName=gridVMs, Path=ItemsSource}" AutoGenerateColumns="False" CanUserAddRows="False" VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Auto"><DataGrid.Columns><DataGridTextColumn Header="VM Name" Binding="{Binding VMName}" IsReadOnly="True" Width="170"/><DataGridTextColumn Header="Migration Type" Binding="{Binding MigrationType}" Width="210"/><DataGridTextColumn Header="Destination Site" Binding="{Binding DestinationSite}" IsReadOnly="True" Width="170"/><DataGridTemplateColumn Header="Destination Compute" Width="250"><DataGridTemplateColumn.CellTemplate><DataTemplate><ComboBox ItemsSource="{Binding DataContext.Computes, RelativeSource={RelativeSource AncestorType=Window}}" DisplayMemberPath="DisplayName" SelectedValuePath="Id" SelectedValue="{Binding DestinationComputeId, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn><DataGridTemplateColumn Header="Destination Folder" Width="220"><DataGridTemplateColumn.CellTemplate><DataTemplate><ComboBox ItemsSource="{Binding DataContext.Folders, RelativeSource={RelativeSource AncestorType=Window}}" DisplayMemberPath="Name" SelectedValuePath="Id" SelectedValue="{Binding DestinationFolderId, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn></DataGrid.Columns></DataGrid>
<GroupBox Grid.Row="2" Header="Migration and Switchover Settings"><StackPanel><WrapPanel><RadioButton x:Name="rbP2Now" Content="Start after transfer is complete" IsChecked="True" Margin="8"/><RadioButton x:Name="rbP2Schedule" Content="Use group schedule from Tab 5" Margin="8"/><RadioButton x:Name="rbP2Defer" Content="Defer switchover" Margin="8"/><TextBox x:Name="txtP2Schedule" Width="1" Height="1" Visibility="Collapsed"/><CheckBox x:Name="chkP2Mac" Content="Retain MAC" IsChecked="True" Margin="8"/><CheckBox x:Name="chkP2HW" Content="Upgrade Virtual Hardware" Margin="8"/><CheckBox x:Name="chkP2Tools" Content="Upgrade VM Tools" Margin="8"/></WrapPanel><WrapPanel><CheckBox x:Name="chkP2Attrs" Content="Migrate Custom Attributes" IsChecked="True" Margin="8"/><CheckBox x:Name="chkP2Tags" Content="Migrate vCenter Tags" IsChecked="True" Margin="8"/><CheckBox x:Name="chkP2Iso" Content="Force unmount ISO images" IsChecked="True" Margin="8"/><CheckBox x:Name="chkP2Security" Content="Replicate Security Tags" Margin="8"/><TextBlock Text="Migration type selected above controls the schedule modes presented on Tab 5." Foreground="#76C7D8" VerticalAlignment="Center" Margin="12,3"/></WrapPanel></StackPanel></GroupBox><Button x:Name="btnApplyGlobal" Grid.Row="3" Content="Apply Storage and Destination Defaults to All Applicable VMs" HorizontalAlignment="Right" MinWidth="330"/></Grid></TabItem>

<TabItem Header="4. Storage"><Grid Margin="8"><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="*"/></Grid.RowDefinitions>
<GroupBox Header="Global Storage Defaults"><Grid><Grid.ColumnDefinitions><ColumnDefinition Width="Auto"/><ColumnDefinition Width="230"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="230"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="230"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="220"/></Grid.ColumnDefinitions><TextBlock Text="Storage Target"/><ComboBox x:Name="cmbGlobalDatastore" Grid.Column="1" DisplayMemberPath="DisplayName"/><TextBlock Grid.Column="2" Text="Standard Policy"/><ComboBox x:Name="cmbGlobalStoragePolicy" Grid.Column="3" DisplayMemberPath="Name"/><TextBlock Grid.Column="4" Text="vTPM Policy"/><ComboBox x:Name="cmbVtpmStoragePolicy" Grid.Column="5" DisplayMemberPath="Name"/><TextBlock Grid.Column="6" Text="Disk Format"/><ComboBox x:Name="cmbGlobalDiskFormat" Grid.Column="7" SelectedIndex="0"><ComboBoxItem>Same format as source</ComboBoxItem><ComboBoxItem>Thin Provision</ComboBoxItem><ComboBoxItem>Thick Provision Lazy Zeroed</ComboBoxItem><ComboBoxItem>Thick Provision Eager Zeroed</ComboBoxItem></ComboBox></Grid></GroupBox>
<Border Grid.Row="1" Background="#10242D" BorderBrush="#76C7D8" BorderThickness="1" CornerRadius="3" Margin="5,2,5,7" Padding="8"><DockPanel><Button x:Name="btnApplyStorageDefaults" Content="Apply Storage Defaults to Included VMs" MinWidth="285" FontWeight="Bold" DockPanel.Dock="Left"/><TextBlock Text="Storage Defaults:" FontWeight="SemiBold" VerticalAlignment="Center" Margin="15,3,4,3"/><TextBlock x:Name="lblStorageDefaultsStatus" Text="Not applied" Foreground="Gold" VerticalAlignment="Center" FontWeight="SemiBold"/><CheckBox x:Name="chkPreserveStorageOverrides" Content="Preserve per-VM policy overrides" IsChecked="True" IsEnabled="True" Margin="18,3" ToolTip="Checked: keep rows marked Per-VM Override. Unchecked: replace those policy overrides with the selected Standard or vTPM global policy when Apply Storage Defaults is selected."/></DockPanel></Border>
<DataGrid x:Name="gridStorageRows" Grid.Row="2" ItemsSource="{Binding ElementName=gridVMs, Path=ItemsSource}" AutoGenerateColumns="False" CanUserAddRows="False" VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Auto"><DataGrid.Columns><DataGridTextColumn Header="VM Name" Binding="{Binding VMName}" IsReadOnly="True" Width="170"/><DataGridTextColumn Header="vTPM" Binding="{Binding Vtpm}" IsReadOnly="True" Width="60"/><DataGridTextColumn Header="Policy Assignment" Binding="{Binding StoragePolicyAssignment}" IsReadOnly="True" Width="150"/><DataGridTemplateColumn Header="Effective Storage Policy" Width="250"><DataGridTemplateColumn.CellTemplate><DataTemplate><ComboBox Tag="{Binding}" ItemsSource="{Binding DataContext.Policies, RelativeSource={RelativeSource AncestorType=Window}}" DisplayMemberPath="Name" SelectedValuePath="Name" SelectedValue="{Binding StoragePolicy, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn><DataGridTemplateColumn Header="Destination Storage" Width="260"><DataGridTemplateColumn.CellTemplate><DataTemplate><ComboBox Tag="{Binding}" ItemsSource="{Binding DataContext.Datastores, RelativeSource={RelativeSource AncestorType=Window}}" DisplayMemberPath="DisplayName" SelectedValuePath="Id" SelectedValue="{Binding DestinationDatastoreId, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn><DataGridTextColumn Header="Disk Format" Binding="{Binding DiskFormat}" Width="190"/><DataGridTextColumn Header="Source Datastore" Binding="{Binding SourceDatastore}" IsReadOnly="True" Width="220"/></DataGrid.Columns></DataGrid>
</Grid></TabItem>

<TabItem Header="5. Scheduler"><Grid Margin="10"><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="*"/><RowDefinition Height="Auto"/></Grid.RowDefinitions>
<GroupBox Header="Per-Mobility Group Scheduler"><StackPanel><TextBlock Text="Scheduling is configured independently for each Mobility Group and is driven by the migration type selected on Tab 3." TextWrapping="Wrap"/><TextBlock Text="HCX Assisted vMotion provides Switchover scheduling only. Replication-assisted vMotion provides independent Transfer and Switchover scheduling." Foreground="#76C7D8" TextWrapping="Wrap"/></StackPanel></GroupBox>
<WrapPanel Grid.Row="1"><Button x:Name="btnRefreshSchedules" Content="Refresh Groups"/><Border Background="#17313D" BorderBrush="#76C7D8" BorderThickness="2" CornerRadius="3" Margin="8,3" Padding="8,3"><StackPanel Orientation="Horizontal"><TextBlock Text="Scheduler:" FontWeight="SemiBold" VerticalAlignment="Center"/><TextBlock x:Name="lblSchedulerStatusTop" Text="Not configured" Foreground="#76C7D8" FontWeight="Bold" VerticalAlignment="Center"/><Button x:Name="btnValidateSchedulesTop" Content="Validate Schedules" FontWeight="Bold" Margin="10,0,0,0"/></StackPanel></Border><Button x:Name="btnImportScheduleCsv" Content="Import Schedule CSV"/><Button x:Name="btnExampleScheduleCsv" Content="Download Example Schedule CSV"/><Button x:Name="btnApplyScheduleAll" Content="Copy Selected Row to All Compatible Groups"/><TextBlock Text="Select one row as the source, then use Copy Selected Row to All Compatible Groups. No multi-row selection is required. No Transfer Schedule writes an empty transfer schedule and does not start a migration. Calendar dates and 15-minute time selections are converted to HCX epoch milliseconds." Foreground="#76C7D8" VerticalAlignment="Center" Margin="12,3"/></WrapPanel>
<DataGrid x:Name="gridP2Schedules" Grid.Row="2" AutoGenerateColumns="False" SelectionMode="Single" SelectionUnit="FullRow" CanUserAddRows="False" VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Auto" EnableRowVirtualization="True" VirtualizingPanel.IsVirtualizing="True" VirtualizingPanel.VirtualizationMode="Recycling" ScrollViewer.CanContentScroll="True" FrozenColumnCount="3"><DataGrid.Columns>
<DataGridTextColumn Header="Group #" Binding="{Binding MobilityGroupNumber}" IsReadOnly="True" Width="70"/><DataGridTextColumn Header="Generated Name" Binding="{Binding GeneratedName}" IsReadOnly="True" Width="220"/><DataGridTextColumn Header="Migration Type" Binding="{Binding MigrationType}" IsReadOnly="True" Width="210"/>
<DataGridTemplateColumn Header="Transfer Mode" Width="145"><DataGridTemplateColumn.CellTemplate><DataTemplate><ComboBox ItemsSource="{Binding TransferModes}" SelectedItem="{Binding TransferMode, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}" IsEnabled="{Binding TransferScheduleSupported}"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn>
<DataGridTemplateColumn Header="Transfer Start Date" Width="145"><DataGridTemplateColumn.CellTemplate><DataTemplate><DatePicker SelectedDate="{Binding TransferStartDate, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}" IsEnabled="{Binding TransferScheduleSupported}" SelectedDateFormat="Short"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn>
<DataGridTemplateColumn Header="Start Time" Width="105"><DataGridTemplateColumn.CellTemplate><DataTemplate><ComboBox ItemsSource="{Binding TimeChoices}" SelectedItem="{Binding TransferStartTime, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}" IsEnabled="{Binding TransferScheduleSupported}"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn>
<DataGridTemplateColumn Header="Transfer Expiry Date" Width="145"><DataGridTemplateColumn.CellTemplate><DataTemplate><DatePicker SelectedDate="{Binding TransferExpiryDate, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}" IsEnabled="{Binding TransferScheduleSupported}" SelectedDateFormat="Short"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn>
<DataGridTemplateColumn Header="Expiry Time" Width="105"><DataGridTemplateColumn.CellTemplate><DataTemplate><ComboBox ItemsSource="{Binding TimeChoices}" SelectedItem="{Binding TransferExpiryTime, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}" IsEnabled="{Binding TransferScheduleSupported}"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn>
<DataGridTemplateColumn Header="Switchover Mode" Width="170"><DataGridTemplateColumn.CellTemplate><DataTemplate><ComboBox ItemsSource="{Binding SwitchoverModes}" SelectedItem="{Binding SwitchoverMode, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn>
<DataGridTemplateColumn Header="Switchover Start Date" Width="155"><DataGridTemplateColumn.CellTemplate><DataTemplate><DatePicker SelectedDate="{Binding SwitchoverStartDate, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}" SelectedDateFormat="Short"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn>
<DataGridTemplateColumn Header="Start Time" Width="105"><DataGridTemplateColumn.CellTemplate><DataTemplate><ComboBox ItemsSource="{Binding TimeChoices}" SelectedItem="{Binding SwitchoverStartTime, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn>
<DataGridTemplateColumn Header="Switchover End Date" Width="155"><DataGridTemplateColumn.CellTemplate><DataTemplate><DatePicker SelectedDate="{Binding SwitchoverEndDate, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}" SelectedDateFormat="Short"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn>
<DataGridTemplateColumn Header="End Time" Width="105"><DataGridTemplateColumn.CellTemplate><DataTemplate><ComboBox ItemsSource="{Binding TimeChoices}" SelectedItem="{Binding SwitchoverEndTime, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"/></DataTemplate></DataGridTemplateColumn.CellTemplate></DataGridTemplateColumn>
<DataGridTextColumn Header="Validation" Binding="{Binding Validation}" IsReadOnly="True" Width="220"/></DataGrid.Columns></DataGrid>
<StackPanel Grid.Row="3" Visibility="Collapsed"><TextBlock x:Name="lblSchedulerStatus" Text="Not configured"/><Button x:Name="btnValidateSchedules" Content="Validate Schedules"/></StackPanel>
</Grid></TabItem>

<TabItem Header="6. Save and Create"><Grid Margin="8"><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/><RowDefinition Height="*"/></Grid.RowDefinitions>
<GroupBox Header="1. Select Mobility Group Data Source"><Grid><Grid.ColumnDefinitions><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="*"/></Grid.ColumnDefinitions><Button x:Name="btnUseCurrentVMs" Grid.Column="0" Content="Use Current VM List" MinWidth="150" ToolTip="Use the VMs and settings currently configured on Tabs 1 through 5."/><Button x:Name="btnP2Import" Grid.Column="1" Content="Import Saved Data" MinWidth="145" ToolTip="Load a previously saved Mobility Group data CSV."/><CheckBox x:Name="chkMasterIncludeP2" Grid.Column="2" Content="Include All Creation VMs" IsThreeState="True" IsChecked="False" ToolTip="Checked includes all creation rows. Cleared excludes all rows. Indeterminate means only some rows are included." Margin="12,3"/><TextBlock Grid.Column="3" Text="Selected Source:" FontWeight="SemiBold" VerticalAlignment="Center" Margin="14,3,4,3"/><TextBox x:Name="txtP2Csv" Grid.Column="4" IsReadOnly="True" Text="No data source selected"/></Grid></GroupBox>
<GroupBox Grid.Row="1" Header="2. Mobility Group Naming"><Grid><Grid.ColumnDefinitions><ColumnDefinition Width="110"/><ColumnDefinition Width="*"/><ColumnDefinition Width="95"/><ColumnDefinition Width="80"/><ColumnDefinition Width="75"/><ColumnDefinition Width="65"/><ColumnDefinition Width="90"/><ColumnDefinition Width="*"/></Grid.ColumnDefinitions><Grid.RowDefinitions><RowDefinition Height="34"/><RowDefinition Height="34"/></Grid.RowDefinitions><TextBlock Text="Base Group Name" VerticalAlignment="Center"/><TextBox x:Name="txtP2Name" Grid.Column="1"/><TextBlock Grid.Column="2" Text="Start Suffix" VerticalAlignment="Center"/><TextBox x:Name="txtP2StartSuffix" Grid.Column="3" Text="1" HorizontalContentAlignment="Center"/><TextBlock Grid.Column="4" Text="Groups" VerticalAlignment="Center"/><ComboBox x:Name="cmbP2GroupCount" Grid.Column="5" Background="#FFFFFF" Foreground="#101820" BorderBrush="#76C7D8" FontWeight="SemiBold" HorizontalContentAlignment="Center" MaxDropDownHeight="360" ScrollViewer.VerticalScrollBarVisibility="Auto"><ComboBox.ItemTemplate><DataTemplate><Border Background="#FFFFFF" Padding="8,4"><TextBlock Text="{Binding}" Foreground="#101820" Background="#FFFFFF" FontWeight="SemiBold" HorizontalAlignment="Center"/></Border></DataTemplate></ComboBox.ItemTemplate><ComboBox.ItemContainerStyle><Style TargetType="ComboBoxItem"><Setter Property="Background" Value="#FFFFFF"/><Setter Property="Foreground" Value="#101820"/><Setter Property="HorizontalContentAlignment" Value="Stretch"/><Style.Triggers><Trigger Property="IsHighlighted" Value="True"><Setter Property="Background" Value="#76C7D8"/><Setter Property="Foreground" Value="#101820"/></Trigger><Trigger Property="IsSelected" Value="True"><Setter Property="Background" Value="#BDEAF2"/><Setter Property="Foreground" Value="#101820"/></Trigger></Style.Triggers></Style></ComboBox.ItemContainerStyle></ComboBox><TextBlock Grid.Column="6" Text="Destination" VerticalAlignment="Center"/><TextBox x:Name="txtP2DestSite" Grid.Column="7" IsReadOnly="True"/><TextBlock Grid.Row="1" Text="Source HCX" VerticalAlignment="Center"/><TextBox x:Name="txtP2Hcx" Grid.Row="1" Grid.Column="1" IsReadOnly="True"/><TextBlock Grid.Row="1" Grid.Column="2" Text="Source Site" VerticalAlignment="Center"/><TextBox x:Name="txtP2SourceSite" Grid.Row="1" Grid.Column="3" Grid.ColumnSpan="5"/></Grid></GroupBox>
<GroupBox Grid.Row="2" Header="3. Validate, Save, Preview, and Create"><Grid><Grid.ColumnDefinitions><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="22"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions><TextBlock Text="Configuration:" FontWeight="SemiBold" VerticalAlignment="Center"/><TextBlock x:Name="lblValidation" Grid.Column="1" Text="Not validated" Foreground="#76C7D8" VerticalAlignment="Center"/><Button x:Name="btnValidate" Grid.Column="2" Content="Validate Configuration"/><TextBlock Grid.Column="4" Text="Creation Readiness:" FontWeight="SemiBold" VerticalAlignment="Center"/><TextBlock x:Name="lblP2Status" Grid.Column="5" Text="Not validated" Foreground="#76C7D8" VerticalAlignment="Center"/><Button x:Name="btnP2Validate" Grid.Column="6" Content="Validate Mobility Groups"/><StackPanel Grid.Column="7" Orientation="Horizontal"><Button x:Name="btnCreateCsv" Content="Save Mobility Group Data" IsEnabled="False"/><Button x:Name="btnP2Preview" Content="Preview Payload" IsEnabled="False"/><Button x:Name="btnP2Create" Content="Create Mobility Group Drafts in HCX" IsEnabled="False" MinWidth="185"/></StackPanel></Grid></GroupBox>
<Expander Grid.Row="3" Header="Optional Final Placement Revisions" IsExpanded="False"><WrapPanel><TextBlock Text="Compute"/><ComboBox x:Name="cmbP2Compute" Width="210" DisplayMemberPath="DisplayName"/><TextBlock Text="Storage"/><ComboBox x:Name="cmbP2Storage" Width="220" DisplayMemberPath="DisplayName"/><TextBlock Text="Standard Policy"/><ComboBox x:Name="cmbP2Policy" Width="220" DisplayMemberPath="Name"/><TextBlock Text="vTPM Policy"/><ComboBox x:Name="cmbP2VtpmPolicy" Width="220" DisplayMemberPath="Name"/><TextBlock Text="Disk"/><ComboBox x:Name="cmbP2Disk" Width="190" SelectedIndex="0"><ComboBoxItem>Same format as source</ComboBoxItem><ComboBoxItem>Thin Provision</ComboBoxItem><ComboBoxItem>Thick Provision Lazy Zeroed</ComboBoxItem><ComboBoxItem>Thick Provision Eager Zeroed</ComboBoxItem></ComboBox><Button x:Name="btnP2Inventory" Content="Load Storage"/><Button x:Name="btnP2Apply" Content="Apply Destination Defaults"/></WrapPanel></Expander>
<DataGrid x:Name="gridP2" Grid.Row="4" AutoGenerateColumns="False" CanUserAddRows="False" VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Auto" EnableRowVirtualization="True" FrozenColumnCount="5"><DataGrid.Columns><DataGridCheckBoxColumn Header="Include" Binding="{Binding Include}" Width="55"/><DataGridTextColumn Header="Group #" Binding="{Binding MobilityGroupNumber}" Width="65"/><DataGridTextColumn Header="VM Name" Binding="{Binding VMName}" Width="150" IsReadOnly="True"/><DataGridTextColumn Header="vTPM" Binding="{Binding Vtpm}" Width="55" IsReadOnly="True"/><DataGridTextColumn Header="Policy Assignment" Binding="{Binding StoragePolicyAssignment}" Width="150" IsReadOnly="True"/><DataGridTextColumn Header="Network Mapping" Binding="{Binding NetworkSummary}" Width="260" IsReadOnly="True"/><DataGridTextColumn Header="Compute" Binding="{Binding Compute}" Width="200"/><DataGridTextColumn Header="Folder" Binding="{Binding Folder}" Width="130"/><DataGridTextColumn Header="Storage" Binding="{Binding Storage}" Width="230"/><DataGridTextColumn Header="Policy" Binding="{Binding StoragePolicy}" Width="220"/><DataGridTextColumn Header="Disk Format" Binding="{Binding DiskFormat}" Width="190"/></DataGrid.Columns></DataGrid>
</Grid></TabItem>

<TabItem Header="7. Logging and Output"><Grid Margin="8"><Grid.RowDefinitions><RowDefinition Height="*"/><RowDefinition Height="Auto"/><RowDefinition Height="Auto"/></Grid.RowDefinitions><GroupBox Header="Operational Log"><TextBox x:Name="txtLog" IsReadOnly="True" TextWrapping="NoWrap" VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Auto" FontFamily="Consolas" FontSize="12"/></GroupBox><GroupBox Grid.Row="1" Header="Output Location"><Grid><Grid.ColumnDefinitions><ColumnDefinition Width="120"/><ColumnDefinition Width="*"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions><TextBlock Text="Default base path"/><TextBox x:Name="txtOutputPath" Grid.Column="1"/><StackPanel Grid.Column="2" Orientation="Horizontal"><Button x:Name="btnBrowseOutput" Content="Browse..."/><Button x:Name="btnApplyOutput" Content="Apply Path"/></StackPanel></Grid></GroupBox><StackPanel Grid.Row="2" Orientation="Horizontal" HorizontalAlignment="Center"><Button x:Name="btnOpenOutput" Content="Open Last CSV"/><Button x:Name="btnOpenRun" Content="Open Run Folder"/><Button x:Name="btnClose" Content="Close"/></StackPanel></Grid></TabItem>
</TabControl>
</Grid></Window>
'@
$script:Window=[Windows.Markup.XamlReader]::Parse($xaml)
$script:MigrationTypes=@('Replication-assisted vMotion','HCX vMotion','HCX Assisted vMotion (Direct)','Bulk Migration','Cold Migration','OS Assisted Migration')
...
$names=@('tabWorkflow','gridNetworkRows','gridP2Schedules','btnRefreshSchedules','btnImportScheduleCsv','btnExampleScheduleCsv','btnApplyScheduleAll','btnValidateSchedules','btnValidateSchedulesTop','lblSchedulerStatus','lblSchedulerStatusTop','gridStorageRows','btnApplyStorageDefaults','lblStorageDefaultsStatus','chkPreserveStorageOverrides','gridDestinationRows','lblPS','lblSourceVCStatus','lblDestinationVCStatus','lblSTA','lblPCLI','lblApi','btnRecheck','btnInstall','txtHcx','txtUser','txtPassword','txtSourceVC','txtSourceVCUser','txtSourceVCPass','txtDestinationVC','txtDestinationVCUser','txtDestinationVCPass','btnConnect','btnDisconnect','btnSaveConnectionJson','btnLoadConnectionJson','lblHcxTopologyStatus','lblHcxDirection','lblHcxServiceMesh','cmbHcxServiceMesh','cmbDestinationSite','cmbGlobalCompute','cmbGlobalDatastore','cmbGlobalFolder','cmbMigrationType','cmbGlobalStoragePolicy','cmbVtpmStoragePolicy','cmbGlobalDiskFormat','txtNetworkMappingCsv','btnImportNetworkMapping','btnClearNetworkMapping','lblNetworkMappingStatus','btnApplyGlobal','gridVMs','btnSelectAllVMs','btnUnselectAllVMs','chkMasterIncludeVMs','btnImport','btnExample','btnExampleNetworkMapping','btnRemove','btnClear','btnRefresh','lblValidation','btnValidate','btnCreateCsv','txtLog','txtOutputPath','btnBrowseOutput','btnApplyOutput','btnOpenOutput','btnOpenRun','btnClose','txtP2Csv','chkMasterIncludeP2','btnUseCurrentVMs','btnP2Import','txtP2Name','cmbP2GroupCount','txtP2StartSuffix','txtP2Hcx','txtP2SourceSite','txtP2DestSite','cmbP2Compute','cmbP2Storage','cmbP2Policy','cmbP2VtpmPolicy','cmbP2Disk','btnP2Inventory','btnP2Apply','rbP2Now','rbP2Schedule','rbP2Defer','txtP2Schedule','chkP2Mac','chkP2HW','chkP2Tools','chkP2Attrs','chkP2Tags','chkP2Iso','chkP2Security','gridP2','lblP2Status','btnP2Validate','btnP2Preview','btnP2Create')
foreach($n in $names){Set-Variable -Scope Script -Name $n -Value $script:Window.FindName($n)}
$requiredControls=@('gridVMs','gridP2','gridP2Schedules','chkMasterIncludeVMs','chkMasterIncludeP2','btnRefreshSchedules','btnValidateSchedules','btnApplyStorageDefaults');$missingControls=@($requiredControls|Where-Object{-not(Get-Variable -Scope Script -Name $_ -ValueOnly -ErrorAction SilentlyContinue)});if($missingControls.Count){throw "Script initialization failed. Missing required UI control(s): $($missingControls -join ', ')."}
function Test-HcxRequiredFunctions {
    $required=@('Convert-HcxNetworkIdCanonical','Resolve-HcxDestinationNetworkMatch','Set-HcxBindingContextCollection','Bind-DestinationControls','Invalidate-Validation','Invalidate-P2Validation','P2-Invalidate','Commit-Grid','Commit-P2Grid','Apply-GlobalSelection','Apply-StorageDefaultsToIncludedVMs','Test-Phase1StoragePolicyConsistency','Validate-Phase1','Initialize-HcxAutomaticDiscovery','Assert-HcxAutomaticDiscoveryCurrent','Get-HcxDiscoveredPayloadTopology','Merge-HcxAutomaticNetworkInventory','Invoke-HcxRest','Assert-HcxSubmissionAuthentication','Refresh-HcxAuthentication','Submit-HcxMobilityGroup','Get-HcxStoragePolicyInventory','Merge-HcxStoragePolicyInventory','Get-P2StoragePolicyDescriptor')
...
$script:gridNetworkRows.ItemsSource=$script:Rows
$script:gridStorageRows.ItemsSource=$script:Rows
$script:gridDestinationRows.ItemsSource=$script:Rows
$script:gridP2Schedules.ItemsSource=$script:P2Schedules
...
$script:Window.AddHandler([Windows.Controls.Button]::ClickEvent,[Windows.RoutedEventHandler]{param($sender,$e);if($e.OriginalSource.Content -eq 'Configure' -and $e.OriginalSource.Tag){Show-NicMappingDialog $e.OriginalSource.Tag}})
$script:gridVMs.Add_CellEditEnding({Invalidate-Validation 'VM grid edited'})
$script:gridStorageRows.Add_CellEditEnding({
    param($sender,$e)
    if($script:ApplyingStorageDefaults){return}
    $row=$e.Row.Item
    $header=[string]$e.Column.Header
    if($row -and $header -eq 'Effective Storage Policy'){$row.StoragePolicyAssignment='Per-VM Override'}
    if($row -and $header -eq 'Destination Storage'){$row.DatastoreSelectionSource='Manual'}
    Set-StorageDefaultsPending "Per-VM storage configuration edited for $($row.VMName)"
})
$script:gridDestinationRows.Add_CellEditEnding({Invalidate-Validation 'Destination configuration edited'})
# Per-row storage selections are captured by TwoWay bindings and grid edit events; invalid ComboBox.DropDownClosedEvent routed handler removed.
...
$script:btnConnect.Add_Click({
    try{
        ...
        Load-HcxInventory;Initialize-HcxAutomaticDiscovery;Merge-HcxStoragePolicyInventory;Merge-HcxAutomaticNetworkInventory;Update-Prerequisites;if($script:P2Rows -and $(@($script:P2Rows).Count) -gt 0){P2-Inventory;Restore-P2SelectionsFromPhaseICsv}
        Show-HcxThemedNotice -Title 'HCX 9.1 Inventory Ready' -Heading 'Connected and inventory loaded' -Kind Success -Message 'HCX Manager and both vCenter inventory sessions are ready.' -Details ("Source VMs: $($(@($script:Inventory.VMs).Count))`nDestination networks: $($(@($script:Inventory.Networks).Count))`nDestination datastores: $($(@($script:Inventory.Datastores).Count))`nDestination compute targets: $($(@($script:Inventory.Computes).Count))`nDestination folders: $($(@($script:Inventory.Folders).Count))`nMigration types: $($(@($script:MigrationTypes).Count))")
...
$script:chkPreserveStorageOverrides.Add_Click({if([bool]$script:chkPreserveStorageOverrides.IsChecked){Set-StorageDefaultsStatus 'Pending - Apply will preserve per-VM overrides' 'Gold'}else{Set-StorageDefaultsStatus 'Pending - Apply will replace per-VM overrides' 'Gold'}})
$script:btnApplyStorageDefaults.Add_Click({try{Apply-StorageDefaultsToIncludedVMs}catch{$detail=Get-HcxDetailedError $_;Log "STORAGE DEFAULT APPLICATION FAILURE: $detail" ERROR;[Windows.MessageBox]::Show($detail,'Apply Storage Defaults','OK','Error')|Out-Null}})
foreach($control in @($script:cmbGlobalDatastore,$script:cmbGlobalStoragePolicy,$script:cmbVtpmStoragePolicy,$script:cmbGlobalDiskFormat)){$control.Add_SelectionChanged({Set-StorageDefaultsPending 'Global storage selection changed'})}
...
function Save-DetailedVmImportSummaryCsv {
    [CmdletBinding()]
    param([Parameter(Mandatory)]$Rows)
    $items=@()
    foreach($r in @($Rows)){
        $finding=if($r.Status -eq 'NotFound'){'Missing from inventory'}elseif($r.Status -eq 'Ambiguous'){'Ambiguous inventory match'}elseif($r.VtpmDetectionStatus -ne 'Detected'){'vTPM detection failed'}elseif($r.PowerState -eq 'PoweredOff'){'Powered off'}else{'Present and powered on'}
        $items+=[pscustomobject][ordered]@{
            CapturedAt=(Get-Date).ToString('o')
            Include=[bool]$r.Include
            MobilityGroupNumber=$r.MobilityGroupNumber
            VMName=$r.VMName
            VMId=$r.VMId
            InventoryStatus=$r.Status
            PowerState=if($r.PowerState){$r.PowerState}else{'Unknown'}
            GuestOS=$r.GuestOS
            CPU=$r.CPU
            MemoryGB=$r.MemoryGB
            vTPM=if($null-eq$r.Vtpm){'Unknown'}else{[string]$r.Vtpm}
            VtpmDetectionStatus=$r.VtpmDetectionStatus
            VtpmDetectionSource=$r.VtpmDetectionSource
            VtpmDetectionDetail=$r.VtpmDetectionDetail
            SourceCompute=$r.SourceCompute
            SourceDatastore=$r.SourceDatastore
            SourceNetworks=$r.SourceNetworks
            NicCount=$r.NicCount
            Finding=$finding
        }
    }
    $path=Join-Path $script:RunDir ('HCX91-VM-Import-Inventory-Findings-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff')+'.csv')
    $items|Export-Csv -LiteralPath $path -NoTypeInformation -Encoding utf8BOM
    $script:LastVmImportReport=$path
    Log "VM import inventory findings CSV saved: $path" PASS
    return [pscustomobject]@{Path=$path;Items=@($items)}
}
function Show-DetailedVmImportSummary($Rows){
 $all=@($Rows);$report=Save-DetailedVmImportSummaryCsv -Rows $all;$items=@($report.Items);foreach($item in $items){if(-not$item.PSObject.Properties['VtpmDetection']){$item|Add-Member -NotePropertyName VtpmDetection -NotePropertyValue $item.VtpmDetectionStatus}}
...
$script:chkMasterIncludeVMs.Add_Click({if(-not$script:MasterIncludeSync){Set-MasterIncludeState VM ([bool]$script:chkMasterIncludeVMs.IsChecked)}})
$script:chkMasterIncludeP2.Add_Click({if(-not$script:MasterIncludeSync){Set-MasterIncludeState P2 ([bool]$script:chkMasterIncludeP2.IsChecked)}})
$script:gridVMs.Add_CurrentCellChanged({Update-MasterIncludeState VM})
$script:gridP2.Add_CurrentCellChanged({Update-MasterIncludeState P2})
$script:btnImportScheduleCsv.Add_Click({try{Import-P2ScheduleCsv}catch{$detail=Get-HcxDetailedError $_;Log $detail ERROR;[Windows.MessageBox]::Show($detail,'Import Scheduler CSV')|Out-Null}})
$script:btnExampleScheduleCsv.Add_Click({try{$p=Save-ExampleScheduleCsv;[Windows.MessageBox]::Show("Example Scheduler CSV created:`n$p",'Scheduler CSV Example')|Out-Null;Invoke-Item $p}catch{$detail=Get-HcxDetailedError $_;Log $detail ERROR;[Windows.MessageBox]::Show($detail,'Scheduler CSV Example')|Out-Null}})
$script:btnRefreshSchedules.Add_Click({Refresh-P2ScheduleRows})
$script:btnValidateSchedules.Add_Click({$null=Validate-P2Schedules})
$script:btnValidateSchedulesTop.Add_Click({$null=Validate-P2Schedules})
$script:btnApplyScheduleAll.Add_Click({
    $selected=$script:gridP2Schedules.SelectedItem;if(-not$selected){[Windows.MessageBox]::Show('Select one Scheduler row by clicking anywhere on the row. The selected row will be used as the source and copied to every compatible group.','Copy Schedule to Groups')|Out-Null;return}
    $compatibleCount=@($script:P2Schedules|Where-Object{[int]$_.MobilityGroupNumber-ne[int]$selected.MobilityGroupNumber -and ($_.SwitchoverModes -contains $selected.SwitchoverMode)}).Count
    $question="Copy the schedule from Group $($selected.MobilityGroupNumber) to $compatibleCount compatible group(s)?`n`nThe selected row is the source. This does not require selecting multiple destination rows."
    if([Windows.MessageBox]::Show($question,'Copy Schedule to Compatible Groups','YesNo','Question') -ne 'Yes'){return}
    foreach($row in @($script:P2Schedules|Where-Object{[int]$_.MobilityGroupNumber-ne[int]$selected.MobilityGroupNumber})){if($row.TransferScheduleSupported-and$selected.TransferScheduleSupported){$row.TransferMode=$selected.TransferMode;$row.TransferStartDate=$selected.TransferStartDate;$row.TransferStartTime=$selected.TransferStartTime;$row.TransferExpiryDate=$selected.TransferExpiryDate;$row.TransferExpiryTime=$selected.TransferExpiryTime};if($row.SwitchoverModes -contains $selected.SwitchoverMode){$row.SwitchoverMode=$selected.SwitchoverMode;$row.SwitchoverStartDate=$selected.SwitchoverStartDate;$row.SwitchoverStartTime=$selected.SwitchoverStartTime;$row.SwitchoverEndDate=$selected.SwitchoverEndDate;$row.SwitchoverEndTime=$selected.SwitchoverEndTime};$row.Validation='Not validated'}
    $script:gridP2Schedules.Items.Refresh();Log "Copied Scheduler settings from Group $($selected.MobilityGroupNumber) to compatible groups." PASS;Set-SchedulerStatus 'Not validated' '#76C7D8';P2-Invalidate
})
$script:gridP2Schedules.Add_CellEditEnding({Set-SchedulerStatus 'Not validated' '#76C7D8';P2-Invalidate})
$script:txtP2Name.Add_TextChanged({if($script:P2Rows.Count -and -not $script:SuppressScheduleRefresh){Refresh-P2ScheduleRows}})
$script:btnSelectAllVMs.Add_Click({
    Commit-Grid
    foreach($row in @($script:Rows)){$row.Include=$true}
    $script:gridVMs.Items.Refresh()
    Invalidate-Validation 'All VM rows selected for inclusion';Update-MasterIncludeState VM
    Log "Select All VMs applied to $(@($script:Rows).Count) row(s)." PASS
})
$script:btnUnselectAllVMs.Add_Click({
    Commit-Grid
    foreach($row in @($script:Rows)){$row.Include=$false}
    $script:gridVMs.Items.Refresh()
    Invalidate-Validation 'All VM rows unselected';Update-MasterIncludeState VM
    Log "Unselect All VMs applied to $(@($script:Rows).Count) row(s)." PASS
})
$script:btnImport.Add_Click({
    try{
        ...
        $script:StorageDefaultsApplied=$true;Set-StorageDefaultsStatus 'Applied during VM import' 'LightGreen'
        ...
        Update-MasterIncludeState VM;Refresh-P2ScheduleRows;Show-DetailedVmImportSummary -Rows @($script:Rows)
    }catch{
...
$script:btnRefresh.Add_Click({
    try{Load-HcxInventory;Initialize-HcxAutomaticDiscovery;Merge-HcxStoragePolicyInventory;Merge-HcxAutomaticNetworkInventory;$names=@($script:Rows|ForEach-Object{$_.VMName});$script:Rows.Clear();foreach($n in $names){$script:Rows.Add((Convert-ToVmRow $n))};Apply-GlobalSelection;$script:StorageDefaultsApplied=$true;Set-StorageDefaultsStatus 'Applied during inventory refresh' 'LightGreen';Log 'HCX inventory and VM discovery refreshed.' PASS}catch{Log $_.Exception.Message ERROR;[Windows.MessageBox]::Show($_.Exception.Message,'Refresh failed','OK','Error')|Out-Null}
})
$script:btnValidate.Add_Click({$null=Validate-SelectedMobilityGroupData})
...
$script:btnUseCurrentVMs.Add_Click({try{Use-CurrentVmListForMobilityGroups}catch{$detail=Get-HcxDetailedError $_;Log $detail ERROR;[Windows.MessageBox]::Show($detail,'Use Current VM List')|Out-Null}})
...
$script:txtP2StartSuffix.Add_TextChanged({P2-Invalidate})
$script:btnP2Apply.Add_Click({P2-Apply});$script:btnP2Validate.Add_Click({P2-Validate});$script:btnP2Preview.Add_Click({P2-Preview});$script:btnP2Create.Add_Click({
    try{
        if(-not(Validate-P2Schedules)){throw 'One or more Mobility Group schedules failed validation.'};if(-not$script:P2Payload){try{$script:Window.Cursor='Wait';$script:P2Payload=@(P2-BuildAll)}finally{$script:Window.Cursor=$null}}
        P2-Create
...
# Final-placement dropdown selections are captured by TwoWay bindings and grid edit events; invalid ComboBox.DropDownClosedEvent routed handler removed.
...
Log 'VM import evidence CSV and Select All / Unselect All VM controls are enabled.' PASS
Log 'HCX storage-profile inventory is enabled, including Datastore Default Storage Policy with value=default and type=default.' PASS
Log 'Configurable Mobility Group starting suffix is enabled. Generated names increment from the selected starting value.' PASS
Log 'Source and destination vCenter connection-state indicators, Scheduler CSV import/example export, robust HH:mm parsing, and tri-state Include All controls are enabled.  Scheduler rows are available immediately from the current imported VM list. HCX Assisted vMotion uses switchover scheduling only; RAV uses independent transfer and switchover schedules selected with calendars and time lists.' PASS
