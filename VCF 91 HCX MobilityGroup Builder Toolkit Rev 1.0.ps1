...
function Invoke-HcxRest([string]$Method,[string]$Path,$Body=$null,[hashtable]$Headers=@{},[switch]$AllowFailure) {
    ...
    catch { if($AllowFailure){return $null}; throw "HCX REST $Method $Path failed: $($_.Exception.Message)" }
}
 ...
 if(-not$script:P2DestinationSegments.Count){Log 'HCX returned no destination NSX segments for Phase II network resolution. Existing Phase I CSV mappings will be retained.' WARN; $phaseISegments=@{}
            foreach($row in @($script:P2Rows)){
                foreach($mapping in @($row.Networks)){
                    ...
                    if($segmentId -notmatch '^/infra/segments/[^/]+$' -and $segmentName){$segmentId='/infra/segments/'+[uri]::EscapeDataString($segmentName);$mapping.DestinationNetworkId=$segmentId}
...
function Get-P2HcxDestinationSegments{
 $uri=$script:Hcx.BaseUri.TrimEnd('/')+'/hybridity/api/v2/inventory/related?hcspUUID=20260913154609621-72b51544-dc1d-4bc3-ac88-28d0231f77c5'
 $body=[ordered]@{
  endpointId='20260913140243147-dc09737d-f038-4de2-b2cb-4c56fa4316e4'
  infraManagerId='31612b42-072b-e50a-02a1-5028ee290ccd'
  categoryType='NETWORK';relatedEntityType='NsxtSegment';inventoryType='NSX'
  entityId='31612b42-072b-e50a-02a1-5028ee290ccd';entityType='NSX'
 ...
 $response=Invoke-HcxRest POST ($uri.Substring($script:Hcx.BaseUri.Length)) ($body|ConvertFrom-Json)
