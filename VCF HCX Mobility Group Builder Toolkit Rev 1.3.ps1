<#
.SYNOPSIS
  VCF 91 HCX MobilityGroup Builder Toolkit Rev 1.3-Production-Import-Fix.ps1
.DESCRIPTION
...
function Invoke-HcxRest([string]$Method,[string]$Path,$Body=$null,[hashtable]$Headers=@{},[switch]$AllowFailure) {
    ...
    catch { $status='';$detail=$_.Exception.Message;try{$status=[int]$_.Exception.Response.StatusCode}catch{};try{if($_.ErrorDetails.Message){$detail=$_.ErrorDetails.Message}}catch{};Log "HCX REST FAILURE: Method='$Method'; Path='$Path'; HTTP='$status'; Response='$detail'." ERROR;if($AllowFailure){return $null};throw "HCX REST $Method $Path failed. HTTP=$status; Response=$detail" }
}
...
function Get-P2ErrorDiagnostic($ErrorRecord){
 $line='';$position='';$stack=''
 ...
 return "Message=$($ErrorRecord.Exception.Message); Line=$line; Position=$position; Stack=$stack"
}
function Get-P2SafeCount($Value){return @($Value).Count}
function P2-Import{
 ...
 try{
  ...
  if((Get-P2SafeCount $csv)-eq0){throw 'CSV is empty.'}
  $script:P2Rows.Clear();$script:P2DestinationSegments=@()
  foreach($r in $csv){
   ...
   $script:P2Rows.Add([pscustomobject]@{
    Include=$true;VMName=$r.VMName;VMId=$r.VMId;Compute=$r.DestinationCompute;ComputeId=$r.DestinationComputeId
    ComputeType=if($r.DestinationComputeType){$r.DestinationComputeType}else{'cluster'}
    MobilityGroupNumber=if($r.MobilityGroupNumber){[int]$r.MobilityGroupNumber}else{1}
    Folder=$r.DestinationFolder;FolderId=$r.DestinationFolderId
    StorageType=if($r.DestinationStorageType){$r.DestinationStorageType}else{'datastore'}
    Storage=$r.DestinationDatastore;StorageId=$r.DestinationDatastoreId
    StoragePolicy=if($r.DestinationStoragePolicy){[string]$r.DestinationStoragePolicy}else{''}
    DiskFormat='Thin Provision';Networks=@($net)
  ...
  $groupValues=@($script:P2Rows|ForEach-Object{[int]$_.MobilityGroupNumber})
  $max=if((Get-P2SafeCount $groupValues)-gt0){[int](($groupValues|Measure-Object -Maximum).Maximum)}else{1}
  if($max-lt1){$max=1};$script:cmbP2GroupCount.SelectedItem=$max
  $script:txtP2Csv.Text=$d.FileName
  Restore-P2SelectionsFromPhaseICsv
  $script:txtP2Hcx.Text=[string]$csv[0].HCXManager
  $script:txtP2DestSite.Text=[string]$csv[0].DestinationSite
  ...
  P2-Invalidate
  Log "Phase II imported $(Get-P2SafeCount $csv) VM(s). Source site was populated from the source vCenter connection." PASS
 }catch{
  $detail=Get-P2ErrorDiagnostic $_;Log "PHASE II IMPORT FAILURE: $detail" ERROR;throw
 }
}
...
function Resolve-P2ImportedNetworks{
 ...
 foreach($row in @($script:P2Rows)){
  foreach($mapping in @($row.Networks)){
   ...
   if(-not$srcType){$issues.Add("$($row.VMName): unsupported source network ID '$srcId' for '$($mapping.SourceNetworkName)'.")}
   if(-not$dstType){$issues.Add("$($row.VMName): unsupported destination network ID '$dstId' for '$($mapping.DestinationNetworkName)'.")}
 ...
 Log "Phase II import network check passed for $(Get-P2SafeCount $script:P2Rows) VM(s); $resolved mapping(s) retained authoritative vCenter or NSX inventory IDs from Phase I." PASS
}
...
function Get-P2HcxDestinationSegments{Log 'Related-inventory lookup skipped; Phase II uses network IDs exported from live vCenter inventory in Phase I.' INFO;return @()}
function Get-P2NativeNetworkMapping($Mapping){
 ...
 $srcType=Get-P2NetworkEntityType $srcId;$dstType=Get-P2NetworkEntityType $dstId
 if(-not$srcType){throw "Unsupported source network ID '$srcId' for '$($Mapping.SourceNetworkName)'."}
 if(-not$dstType){throw "Unsupported destination network ID '$dstId' for '$($Mapping.DestinationNetworkName)'."}
...
function P2-Validate {
    ...
    foreach($row in @($script:P2Rows|Where-Object Include)){
        foreach($mapping in @($row.Networks)){
            if(-not(Get-P2NetworkEntityType ([string]$mapping.DestinationNetworkId))){
                $errors.Add("$($row.VMName): destination network '$($mapping.DestinationNetworkName)' has an unsupported inventory ID '$($mapping.DestinationNetworkId)'.")
            }
        }
    }
    if($errors.Count){
...
$script:btnP2Import.Add_Click({try{P2-Import}catch{$detail=Get-P2ErrorDiagnostic $_;Log $detail ERROR;[Windows.MessageBox]::Show($detail,'Phase II Import')|Out-Null}})
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAow8Ff5dP9N6Dl
# m+CQCQpFMOVnQ+O78sf1XXcXGXIORKCCA0QwggNAMIICKKADAgECAhB49EDlG7pj
# sEN9sx7wOLzaMA0GCSqGSIb3DQEBCwUAMDgxNjA0BgNVBAMMLUhDWDkxIE1vYmls
# aXR5IENTViBCdWlsZGVyIExvY2FsIENvZGUgU2lnbmluZzAeFw0yNjA5MTMxNDA4
# NTNaFw0yOTA5MTMxNDE4NTNaMDgxNjA0BgNVBAMMLUhDWDkxIE1vYmlsaXR5IENT
# ViBCdWlsZGVyIExvY2FsIENvZGUgU2lnbmluZzCCASIwDQYJKoZIhvcNAQEBBQAD
# ggEPADCCAQoCggEBAN//o4bk794pVqnzv9Yby6eMkSeqRrLO1soCx6D5brkC6GGk
# vYFGBq4haN/xkBcWw0J4Zex36ahDiiWahIKie0Si1MOhf7aTEBbZjJWM6FHXmzaz
# I83U4bCzOb+dj7cpsHx6U3USKs6mr/DDjQVeGzkC+wj4sBz+E9LoVR+z8uly7PHb
# MP/wA4WGNuAsrVkbADtaxdXsfvs85dhDM39uXICNfZ73z+99QK4D4/4HBEWvw0Sa
# WFNSPIUNR8Vteb5+GQCboV1pQtONOkjGa6WFaOSKioa55UgDSWFGzWdvpd5axeox
# UtmiYNlpyERgR+gOgAUzLROm6H3AU+A/ooiAeHkCAwEAAaNGMEQwDgYDVR0PAQH/
# BAQDAgeAMBMGA1UdJQQMMAoGCCsGAQUFBwMDMB0GA1UdDgQWBBRObK7hXfMfvOkm
# 2fgEEyuiOC7lUjANBgkqhkiG9w0BAQsFAAOCAQEAAxwGdkWn6++oU7h6tGGkjDge
# HIaZOBoFfa8nFo1nrK6tQqUAmsvs87pD3AVxC+AidstOVgO0iDWHTJpG2RRUe1hB
# q2BjTfuAu99exG9zTX+RWGjddl8rnxPX5noL3P04K07K3rbaB0OFXuh4bV2p6Ipw
# 98wsCiPXQ+OeOH1nffovKXmraZgCItxYBqvAE1Rw6e3E35owStqKQjEE9iUGTnUH
# DgxUZQj0eZBNgHnIdtk0dGdukJXyDnFwuc4xg1dFGbeBBJ1w0HkJ/z1yx0ZzyFjS
# 0RrIoF0szCOP5m+LR2nWmh36sBy5bWMLJUzyAtEfwkKSclm7bHxDmgZdzCSUizGC
# Af4wggH6AgEBMEwwODE2MDQGA1UEAwwtSENYOTEgTW9iaWxpdHkgQ1NWIEJ1aWxk
# ZXIgTG9jYWwgQ29kZSBTaWduaW5nAhB49EDlG7pjsEN9sx7wOLzaMA0GCWCGSAFl
# AwQCAQUAoIGEMBgGCisGAQQBgjcCAQwxCjAIoAKAAKECgAAwGQYJKoZIhvcNAQkD
# MQwGCisGAQQBgjcCAQQwHAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJ
# KoZIhvcNAQkEMSIEIDteCBft7+ghGbIBVW9y6JAZRoJR7WRUBxx/jiwQYqpKMA0G
# CSqGSIb3DQEBAQUABIIBAJVXFjAtj9Go/8bPqlejNgblUFtyJxlu/kH4SfTk+2Jr
# oxG92HTDv5zufn7GyI0MQ50N0UuXcSFNkqU0IHiERtnGx2goxtzi135ARdI/QQlh
# zra5a6cf5IRsSgrTHN8vCDcb3N95a47uLK0Am1fDRtij75ZYrjuCfbTuKJTkOauP
# 8G2aVyVtuA8Ay5M0HeC/J+I/6mxDZdXvNTI/wrRShyitRnV/sMrlW/6Xlc1jSJVS
# 3C523p/oL3yrFhGpQCqS7tLW0XohPCUKFQcvA6M6H+Hac+RuN9yYkVcW0OFRisUW
# mr14ZV+M8+8gxKfCwHK1RartWhYO9HG3ttqWr3ZjurU=
