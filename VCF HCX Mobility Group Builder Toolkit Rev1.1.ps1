<#
.SYNOPSIS
  VCF 91 HCX MobilityGroup Builder Toolkit Rev 1.1.ps1
.DESCRIPTION
...
$script:Hcx = [ordered]@{ BaseUri=''; Session=$null; Connected=$false; Version=''; User=''; SitePairs=@(); EndpointProfile=''; Headers=@{}; Topology=$null; HcsUUID=''; Discovery=@{} }
...
function Get-HcxHttpErrorDetail($ErrorRecord) {
    $status=''; $detail=[string]$ErrorRecord.Exception.Message
    try{$status=[int]$ErrorRecord.Exception.Response.StatusCode}catch{}
    try{if($ErrorRecord.ErrorDetails.Message){$detail=[string]$ErrorRecord.ErrorDetails.Message}}catch{}
    try{
        if([string]::IsNullOrWhiteSpace($detail) -and $ErrorRecord.Exception.Response){
            $stream=$ErrorRecord.Exception.Response.GetResponseStream()
            if($stream){$reader=[IO.StreamReader]::new($stream);try{$detail=$reader.ReadToEnd()}finally{$reader.Dispose()}}
        }
    }catch{}
...
function Invoke-HcxRest([string]$Method,[string]$Path,$Body=$null,[hashtable]$Headers=@{},[switch]$AllowFailure) {
    ...
    $requestJson=$null
    if($null -ne $Body){$requestJson=$Body|ConvertTo-Json -Depth 50 -Compress;$p.Body=$requestJson;$p.ContentType='application/json'}
    try { Invoke-RestMethod @p }
    catch {
        $e=Get-HcxHttpErrorDetail $_
        $safeUri=$uri -replace '(?i)(token|password|authorization)=[^&]+','$1=********'
        Log "HCX REST FAILURE: Method='$Method'; Uri='$safeUri'; HTTP='$($e.Status)'; Response='$($e.Detail)'." ERROR
        if($requestJson){Log "HCX REST REQUEST BODY: $requestJson" WARN}
        if($AllowFailure){return $null}
        throw "HCX REST $Method $Path failed. HTTP=$($e.Status); Response=$($e.Detail); Exception=$($_.Exception.Message)"
    }
}
function Connect-Hcx91Rest([string]$Fqdn,[string]$User,[string]$Password) {
    ...
    $script:Hcx.Topology=$null;$script:Hcx.HcsUUID='';$script:Hcx.Discovery=@{};$script:P2DestinationSegments=$null
...
function Resolve-P2ImportedNetworks{
 if(-not$script:Hcx.Connected){throw 'Connect to the source HCX Manager before importing Phase II network mappings.'}
 $script:P2DestinationSegments=@(Get-P2HcxDestinationSegments)
 if(-not$script:P2DestinationSegments.Count){Log 'HCX returned no destination NSX segments for Phase II network resolution. Existing Phase I CSV mappings will be retained.' WARN; $phaseISegments=@{}
            foreach($row in @($script:P2Rows)){
                foreach($mapping in @($row.Networks)){
                    $segmentName=[string]$mapping.DestinationNetworkName
                    $segmentId=[string]$mapping.DestinationNetworkId
                    if($segmentId -notmatch '^/infra/segments/[^/]+$' -and $segmentName){Log "Phase I mapping '$segmentName' does not contain an authoritative NSX segment path; no synthetic ID will be generated." WARN}
                    if($segmentId -match '^/infra/segments/[^/]+$'){$phaseISegments[$segmentId]=[pscustomobject]@{name=$segmentName;entityId=$segmentId;entityType='NsxtSegment'}}
                }
            }
            $fallbackSegments=@($phaseISegments.Values)
            Log ("Using {0} Phase I NSX segment mapping(s) because HCX enumeration returned none." -f $fallbackSegments.Count) WARN
            $script:P2DestinationSegments=@($fallbackSegments)}
 ...
 foreach($row in @($script:P2Rows)){
  foreach($mapping in @($row.Networks)){
   ...
   if($matches.Count-eq1){
    $segment=$matches[0]
    if($segment.name-cne$requestedName -or [string]$mapping.DestinationNetworkId-cne[string]$segment.entityId){
     Log "Phase II network normalized for $($row.VMName): requested '$requestedName' -> HCX '$($segment.name)' [$($segment.entityId)]." WARN
    }
    $mapping.DestinationNetworkName=[string]$segment.name
    $mapping.DestinationNetworkId=[string]$segment.entityId
    if($mapping.PSObject.Properties['DestinationNetworkType']){$mapping.DestinationNetworkType=[string]$segment.entityType}
    else{$mapping|Add-Member -NotePropertyName DestinationNetworkType -NotePropertyValue ([string]$segment.entityType)}
    if($mapping.PSObject.Properties['MatchStatus']){$mapping.MatchStatus='ExactMatch-CaseInsensitive'}
   }elseif($matches.Count-eq0){
    $issues.Add("$($row.VMName): no destination NSX segment matched '$requestedName' using case-insensitive comparison.")
   }else{
    $issues.Add("$($row.VMName): multiple destination NSX segments matched '$requestedName' using case-insensitive comparison.")
   }
  }
 ...
 if($issues.Count){foreach($issue in $issues){Log $issue ERROR};throw ($issues-join[Environment]::NewLine)}
 Log "Phase II import network check passed for $($script:P2Rows.Count) VM(s) using authoritative HCX segment IDs." PASS
}
...
function Get-P2ObjectPropertyValue($Object,[string[]]$Names){
 foreach($name in $Names){$p=$Object.PSObject.Properties[$name];if($p -and $null-ne$p.Value -and -not[string]::IsNullOrWhiteSpace([string]$p.Value)){return $p.Value}}
 return $null
}
function Get-P2RecursiveObjects($Object){
 $items=[Collections.Generic.List[object]]::new()
 function Add-Node($Node){
  if($null-eq$Node){return}
  if($Node -is [string] -or $Node.GetType().IsPrimitive){return}
  if($Node -is [Collections.IDictionary]){foreach($v in $Node.Values){Add-Node $v};return}
  if($Node -is [Collections.IEnumerable] -and $Node -isnot [pscustomobject]){foreach($v in $Node){Add-Node $v};return}
  $items.Add($Node)
  foreach($p in $Node.PSObject.Properties){if($p.Value -isnot [string]){Add-Node $p.Value}}
 }
 Add-Node $Object
 return @($items)
}
function Initialize-P2HcxTopology{
 if($script:Hcx.Topology){return $script:Hcx.Topology}
 if(-not$script:Hcx.Connected){throw 'HCX must be connected before topology discovery.'}
 $all=[Collections.Generic.List[object]]::new();$attempted=[Collections.Generic.List[string]]::new()
 foreach($path in @('/hybridity/api/endpoints','/hybridity/api/v2/endpoints','/hybridity/api/sitepairs','/hybridity/api/sites','/hybridity/api/service-mesh','/hybridity/api/servicemesh')){
  try{$r=Invoke-HcxRest GET $path -AllowFailure;if($r){foreach($o in @(Get-P2RecursiveObjects $r)){$all.Add($o)};$attempted.Add("$path=success") }else{$attempted.Add("$path=no-response")}}catch{$attempted.Add("$path=$($_.Exception.Message)")}
 }
 $srcName=$script:txtSourceVC.Text.Trim();$dstName=$script:txtDestinationVC.Text.Trim()
 function Find-Endpoint([string]$needle){
  $matches=@($all|Where-Object{
   $names=@([string](Get-P2ObjectPropertyValue $_ @('resourceName','endpointName','name','displayName','url','hostname','fqdn')))
   @($names|Where-Object{$_ -and (($_ -ieq $needle) -or ($_ -like "*$needle*"))}).Count -gt 0 -and
   (Get-P2ObjectPropertyValue $_ @('endpointId','resourceId','computeResourceId','id','uuid','uid'))
  })
  if($matches.Count -eq 1){return $matches[0]}
  if($matches.Count -gt 1){return $matches|Sort-Object{if(([string](Get-P2ObjectPropertyValue $_ @('resourceName','endpointName','name'))) -ieq $needle){0}else{1}}|Select-Object -First 1}
  return $null
 }
 $src=Find-Endpoint $srcName;$dst=Find-Endpoint $dstName
 $mesh=@($all|Where-Object{(Get-P2ObjectPropertyValue $_ @('serviceMeshId','servicemeshId','serviceMeshUuid','id')) -and (([string](Get-P2ObjectPropertyValue $_ @('name','type','entityType'))) -match '(?i)mesh')})|Select-Object -First 1
 if(-not$src -or -not$dst){
  $artifact=Join-Path (script:Get-P2ArtifactFolder) ('HCX91-Topology-Discovery-'+(Get-Date -Format 'yyyyMMdd-HHmmss-fff')+'.json')
  [pscustomobject]@{SourceVCenter=$srcName;DestinationVCenter=$dstName;Attempts=@($attempted);Objects=@($all)}|ConvertTo-Json -Depth 30|Set-Content -LiteralPath $artifact -Encoding utf8
  throw "HCX topology discovery could not uniquely resolve source '$srcName' and destination '$dstName'. Discovery was saved to '$artifact'. No hard-coded lab identifiers were used."
 }
 $srcEndpoint=[string](Get-P2ObjectPropertyValue $src @('endpointId','id','uuid','uid'))
 $dstEndpoint=[string](Get-P2ObjectPropertyValue $dst @('endpointId','id','uuid','uid'))
 $srcResource=[string](Get-P2ObjectPropertyValue $src @('resourceId','computeResourceId','infraManagerId','vcUuid','instanceUuid'))
 $dstResource=[string](Get-P2ObjectPropertyValue $dst @('resourceId','computeResourceId','infraManagerId','vcUuid','instanceUuid'))
 $hcs=[string](Get-P2ObjectPropertyValue $src @('hcsUUID','hcspUUID','sitePairId','sitePairUuid','pairId'))
 if(-not$hcs){$hcs=[string](Get-P2ObjectPropertyValue $dst @('hcsUUID','hcspUUID','sitePairId','sitePairUuid','pairId'))}
 $meshId=[string](Get-P2ObjectPropertyValue $mesh @('serviceMeshId','servicemeshId','serviceMeshUuid','id'))
 $script:Hcx.Topology=[pscustomobject]@{
  HcsUUID=$hcs;ServiceMeshId=$meshId
  Source=[ordered]@{endpointId=$srcEndpoint;endpointName=[string](Get-P2ObjectPropertyValue $src @('endpointName','name','displayName'));endpointType='VC';resourceId=$srcResource;resourceName=$srcName;resourceType='VC';computeResourceId=$srcResource}
  Destination=[ordered]@{endpointId=$dstEndpoint;endpointName=[string](Get-P2ObjectPropertyValue $dst @('endpointName','name','displayName'));endpointType='VC';resourceId=$dstResource;resourceName=$dstName;resourceType='VC';computeResourceId=$dstResource}
 }
 $script:Hcx.HcsUUID=$hcs
 Log "HCX topology discovered: SourceEndpoint='$srcEndpoint'; SourceResource='$srcResource'; DestinationEndpoint='$dstEndpoint'; DestinationResource='$dstResource'; HcsUUID='$hcs'; ServiceMesh='$meshId'." PASS
 return $script:Hcx.Topology
}
function Get-P2HcxDestinationSegments{
 $topology=Initialize-P2HcxTopology
 if([string]::IsNullOrWhiteSpace([string]$topology.HcsUUID)){throw 'HCX topology discovery did not return an HCS/site-pair UUID. Network discovery cannot continue safely.'}
 if([string]::IsNullOrWhiteSpace([string]$topology.Destination.endpointId) -or [string]::IsNullOrWhiteSpace([string]$topology.Destination.resourceId)){throw 'HCX topology discovery did not return the destination endpoint and resource identifiers.'}
 $body=[ordered]@{
  endpointId=[string]$topology.Destination.endpointId
  infraManagerId=[string]$topology.Destination.resourceId
  categoryType='NETWORK';relatedEntityType='NsxtSegment';inventoryType='NSX'
  entityId=[string]$topology.Destination.resourceId;entityType='NSX'
  filter=[ordered]@{entityTypeLegacy=$true;excludeLowerBackings=$true}
  pageParameters=[ordered]@{pageNumber=1;pageSize=500;sortBy=@([ordered]@{field='name';direction='ASC'})}
 }
 $last=$null
 foreach($queryName in @('hcsUUID','hcspUUID')){
  $path='/hybridity/api/v2/inventory/related?{0}={1}' -f $queryName,[uri]::EscapeDataString([string]$topology.HcsUUID)
  try{$response=Invoke-HcxRest POST $path $body;if($response){$items=@(Get-ArrayFromResponse $response);if($items.Count){Log "HCX destination segment discovery succeeded using $queryName with $($items.Count) item(s)." PASS;return $items}}}catch{$last=$_;Log "HCX destination segment discovery using $queryName failed: $($_.Exception.Message)" WARN}
 }
 if($last){throw $last}
 return @()
}
    ...
    foreach($row in @($script:P2Rows|Where-Object Include)){
        foreach($mapping in @($row.Networks)){
            ...
            if(-not[string]::IsNullOrWhiteSpace($name)){
                $mapping.DestinationNetworkId=''
                Log ("Did not fabricate an NSX segment ID for Phase I network '{0}' for VM '{1}' to '{2}'." -f $name,$row.VMName,$mapping.DestinationNetworkId) PASS
            }
        }
    }
}
...
function P2-Build{
 ...
 # Resolve production HCX topology from the active authenticated session.
 $topology=Initialize-P2HcxTopology
 $source=$topology.Source
 $destination=$topology.Destination
 if([string]::IsNullOrWhiteSpace([string]$topology.ServiceMeshId)){throw 'HCX topology discovery did not return a service mesh ID. Payload construction stopped before draft creation.'}
 ...
 $groupDefaults=[ordered]@{
  source=$source;destination=$destination;migrationType='xVMotion';servicemeshId=[string]$topology.ServiceMeshId
 ...
 foreach($row in $included){
  ...
  foreach($adapter in $adapters){
   ...
   Log "Phase II VM placement: VM=$($row.VMName); Compute=$($row.Compute); ComputeId=$(Convert-P2MoRef ([string]$row.ComputeId)); ServiceMesh=$($topology.ServiceMeshId)." PASS
  }
